...
Building on this causal Lindhard foundation, we introduce a scalar Zeta-RPA pathway that dresses the Coulomb interaction with a Riemann-zeta weight $W_\zeta(\Gamma,r_s,\tau)$, recovering ordinary scalar RPA as $\Gamma\to 0$ while retaining continuous dielectric roots and a modified Landau-damping spectrum at strong coupling.
\end{abstract}
...
The two-component manuscript pipelines of Secs.~\ref{sec:multicomponent}--\ref{sec:results}
remain on the undressed RPA tensor Eqs.~(\ref{rpa-responses});
the present section isolates the scalar Zeta-RPA diagnostic as the controlled strong-coupling extension
of the finite-temperature Lindhard foundation.
...
in which the scalar Zeta-RPA pathway is the intended production dress;
outside that window the undressed scalar RPA of Eq.~(\ref{eq:scalar-rpa}) is recovered
by the $W_\zeta\to 1$ identity without additional parameters.
