...
The induction of a distinct peak in $S_{ei}$ [Fig.~\ref{fig:Sq-gamma-ei} inset], despite its small magnitude ($\mathcal{O}(10^{-4})$), quantifies the exact electron-drag effect where the light electron fluid adheres to the heavy-ion acoustic manifold.
...
\begin{itemize}
    ...
    The filled color field uses $N_{\mathrm{fill}}=200$ log-uniform levels and white isolines $N_{\mathrm{line}}=72$ log-uniform contours, $S_k=S_{\min}(S_{\max}/S_{\min})^{k/N}$, so that successive isolines are equally spaced in $\log_{10} S$; panel~\textbf{(c)} uses the bold cross-channel isoline profile of Fig.~\ref{fig:chi_ei_im}.
