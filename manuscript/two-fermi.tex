...
on a dense $(\bar{q}, \bar{\omega})$ mesh with $\bar{\omega} \in [0.01, 3.0]$ in electron Fermi units.
...
Rendered using a unified high-contrast logarithmic color scale, these panels uncover the raw kinematic terrain of the two-fermion plasma.
Crucially, due to the extreme mass asymmetry ($m_i/m_e \gg 1$), the ion response manifolds in Figs.~\ref{fig:bare-lindhard}(c) and (d) are scaled out to the $\bar{q} = 50.0$ horizon.
This coordinate alignment successfully restores the kinematic symmetry, allowing a direct visual comparison of the single-particle continuum boundaries and causality-driven profiles against their electron counterparts.

Figure~\ref{fig:structure-factors} subsequently illustrates how the collective random-phase approximation (RPA) dynamics reshape this bare lake into sharp, localized collective modes ($S_{ee}$, $S_{ii}$, $S_{ei}$)
obtained from the same run, each rendered as a separate logarithmic contour map on the causal evaluation pipeline above.
Panels~\textbf{(a)} and~\textbf{(c)} use the low-momentum sector $\bar{q} \in [0.1, 4.0]$, while panel~\textbf{(b)} extends $S_{ii}$ to the macroscopic horizon $\bar{q} = 50.0$.
...
The color field uses $N_{\mathrm{fill}}=200$ uniformly log-spaced levels and the white isolines
...
Second, $S_{ii}(\bar{q}, \bar{\omega})$ [Fig.~\ref{fig:S_ii}] is confined to the extreme low-frequency band ($\bar{\omega} \rightarrow 0$),
...
and the structure factor weight concentrates in the acoustic window rather than tracking the electron plasmon dispersion; the extended $\bar{q}$ axis in Fig.~\ref{fig:S_ii} confirms that this low-frequency manifold persists across the macroscopic momentum range.
...
Figures~\ref{fig:t0-re-2d} and \ref{fig:t0-im-2d} visualize the causality-pinned $T=0$ topology derived in Appendix~\ref{sec:Lindhard-function} over the $(\bar{q}, \bar{\omega})$ plane, where the logarithmic singularity in the real part forms a sharp topological ridge. When the numerical extraction is compared along the $\bar{q}=1$ slice [Fig.~\ref{fig:t0-1d-slice}], the engine output (markers) tracks a well-defined, repeatable trajectory, while the contour-based textbook reference (solid curves) occupies a distinct manifold. As shown in the error analysis of Fig.~\ref{fig:t0-1d-slice}, the absolute errors in $\Re \chi^L$ remain at the $\mathcal{O}(1)$ level, whereas the imaginary errors are confined to the truncation floor of the sinc evaluation.
...
\begin{itemize}
    ...
    \item \textbf{FIG. 3.} The non-interacting base manifolds of the finite-temperature Lindhard response functions $\chi_{st}^L(\bar{q}, \bar{\omega})$ at $r_s = 1.0$ and $T = 10{,}000\,\mathrm{K}$.
    Panels \textbf{(a)}--\textbf{(b)} show the real and imaginary parts for the electron channel, panels \textbf{(c)}--\textbf{(d)} for the ion channel (kinematically scaled to an expanded horizon $\bar{q} \in [0, 50.0]$ and an extreme low-frequency zoom $\bar{\omega} \in [0, 0.20]$ to capture the acoustic band), and panels \textbf{(e)}--\textbf{(f)} for the cross-species channel.
    The unified color coding mapping the response magnitudes demonstrates absolute numerical continuity and smooth causality-constrained profiles entirely free from legacy grid artifacts.

    \item \textbf{FIG. 4.} Dynamic structure factors from the MosaiQ-Lindhard deterministic sandbox at $r_s = 1.0$ and $T = 10{,}000\,\mathrm{K}$.
    Panels~\textbf{(a)}, \textbf{(b)}, and~\textbf{(c)} show $S_{ee}$, $S_{ii}$, and $S_{ei}$, respectively, as logarithmic contour maps of $|S_{st}(\bar{q}, \bar{\omega})|$ on the causal $(\bar{q}, \bar{\omega})$ mesh ($\bar{\omega} \in [0.01, 3.0]$ in electron Fermi units for the electron and cross channels).
    Panels~\textbf{(a)} and~\textbf{(c)} cover $\bar{q} \in [0.1, 4.0]$.
    Panel~\textbf{(b)} displays the ion--ion channel, subjected to a kinematic scale extraction ($\bar{q} \le 50.0$, $\bar{\omega} \le 0.20$) to perfectly frame the low-frequency ion-acoustic collective mode.
    ...
    The filled color field uses $N_{\mathrm{fill}}=200$ log-uniform levels and the white isolines $N_{\mathrm{line}}=72$ log-uniform contours, $S_k=S_{\min}(S_{\max}/S_{\min})^{k/N}$, so that successive isolines are equally spaced in $\log_{10} S$.
...
\panelpagefigure[fig:Sq-gamma]{../output/Sq_gamma_ee.pdf}{Fig.~7(a)}{fig:Sq-gamma-ee}
\panelpagefigure{../output/Sq_gamma_ii.pdf}{Fig.~7(b)}{fig:Sq-gamma-ii}
\panelpagefigure{../output/Sq_gamma_ei.pdf}{Fig.~7(c)}{fig:Sq-gamma-ei}
