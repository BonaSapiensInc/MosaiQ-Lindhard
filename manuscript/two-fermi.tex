...
This relation can also be derived in a different way as in Appendix~\ref{sec:Gouedard-Deutsch}.
...
This formulation bypasses the infinite Matsubara pole barrier entirely:
...
the choice ensures the Cauchy principal value integration error of order $10^{-259}$,
which is accurate enough to eliminate the numerical instabilities from the poles of $f\left( x \right)$.
...
Figure~\ref{fig:multi-component-Sqw} summarizes the three diagonal and cross-channel structure factors
...
First, $S_{ee}(\bar{q}, \bar{\omega})$ [Fig.~\ref{fig:multi-component-Sqw}, left]
exhibits a sharp plasmon peak in the high-frequency, high-momentum sector.
...
Second, $S_{ii}(\bar{q}, \bar{\omega})$ [Fig.~\ref{fig:multi-component-Sqw}, center]
is confined to the extreme low-frequency band ($\bar{\omega} \rightarrow 0$),
...
Third, $S_{ei}(\bar{q}, \bar{\omega})$ [Fig.~\ref{fig:multi-component-Sqw}, right]
provides the decisive smoking gun for intercomponent screening and entanglement.
...
at an error class of order $10^{-259}$, thereby injecting spurious numerical noise
...
The search bracket is anchored by the classical Bohm--Gross long-wavelength dispersion relation,
which provides a physics-informed estimate of the plasmon frequency at each $\bar{q}$
and confines the frequency scan to the collective sector.
...
The $f$-sum rule and the compressibility rule are satisfied within controlled integration tolerance
at wave vectors selected for the available spectral weight in the $S_{ee}$ channel.
...
within the same tolerance class.
...
at an error class of order $10^{-259}$.
This maps the calculation into a strictly constrained physical space in which
$\Re\chi^L$ is uniquely determined by the dissipative content of $\Im\chi^L$,
bypassing the infinite Matsubara pole barrier and the analytic inconsistency
...
\section{The zero temperature Lindhard function}
\label{sec:Lindhard-function}

At zero temperature the Lindhard function is well known~\cite{Lindhard1954}.
\begin{eqnarray}
\chi\left( \bar{q}, \bar{\omega} \right) 
&=& \Re \chi\left( \bar{q}, \bar{\omega} \right) + i \Im \chi\left( \bar{q}, \bar{\omega} \right)
\\
\frac{\Re \chi\left( \bar{q}, \bar{\omega} \right)}{\hbar D\left( \epsilon_\mathrm{F} \right)}
&=& \frac{1}{2\bar{q}} \left[ \bar{q} 
+ \frac{1}{2} \left( 1 - \xi_-^2\left( \bar{\omega} \right) \right)
\ln\left\vert  \frac{\xi_-\left( \bar{\omega} \right) - 1}{\xi_-\left( \bar{\omega} \right) + 1} \right\vert
+ \frac{1}{2} \left( 1 - \xi_+^2\left( \bar{\omega} \right) \right)
\ln\left\vert  \frac{\xi_+\left( \bar{\omega} \right) - 1}{\xi_+\left( \bar{\omega} \right) + 1} \right\vert
\right]
\nonumber \\
\frac{\Im \chi\left( \bar{q}, \bar{\omega} \right)}{\hbar D\left( \epsilon_\mathrm{F} \right)}
&=& \frac{\pi}{4 \bar{q}}
\begin{cases}
0 , & \xi_-^2 \ge 1  \\
...
\section{Summation in the real part of finite-temperature susceptibility}
...
the real part is obtained with a sinc-quadrature error bound of order $10^{-259}$
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.32\textwidth]{../output/S_ee_plot.png}\hfill
\includegraphics[width=0.32\textwidth]{../output/S_ii_plot.png}\hfill
\includegraphics[width=0.32\textwidth]{../output/S_ei_plot.png}
\caption{Dynamic structure factors from the SOPHUS-Lindhard deterministic sandbox
at $r_s = 1.0$ and $T = 10{,}000\,\mathrm{K}$.
Left: $S_{ee}(\bar{q}, \bar{\omega})$;
center: $S_{ii}(\bar{q}, \bar{\omega})$;
right: $S_{ei}(\bar{q}, \bar{\omega})$.
Each panel combines a two-dimensional contour map (upper) and a three-dimensional surface plot (lower)
on the same $(\bar{q}, \bar{\omega})$ grid.}
\label{fig:multi-component-Sqw}
\end{figure}
...
The dispersion (solid line) diverges from the classical Bohm--Gross limit (dashed line) due to quantum screening,
