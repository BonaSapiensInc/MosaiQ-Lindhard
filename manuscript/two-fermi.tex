...
Furthermore, we resolve the longstanding $\mathcal{O}(1)$ topological discrepancy in the zero-temperature limit of the Lindhard function. By introducing the concept of a \emph{reverse Dedekind cut}---arising from the intrinsic zero-point avoidance of fermionic Matsubara frequencies---we demonstrate that the macroscopic step function must strictly evaluate to $f(0)=1/2$ at the Fermi level to preserve Kramers--Kr\"onig causality. Guided by this Stratonovich imperative, we propose a singularity excision algorithm that analytically subdues the catastrophic numerical divergence, extending the IEEE~754 double-precision limit of the causal response deep into extreme cryogenic regimes.
Building on this causal Lindhard foundation, we introduce a Zeta-RPA pathway that dresses the Coulomb interaction with a Riemann-zeta weight $W_\zeta(\Gamma,r_s,\tau)$, recovering ordinary RPA as $\Gamma\to 0$ while retaining continuous dielectric roots and a modified Landau-damping spectrum at strong coupling. Extreme mass asymmetry then promotes the construction to a two-component asymmetric zeta matrix with independent diagonal regimes $(\Gamma_e,\Gamma_i)$ and an arithmetic-mean cross channel $\Gamma_{ei}=(\Gamma_e+\Gamma_i)/2$, leaving the bare Lindhard susceptibilities untouched.
\end{abstract}
%\pacs{52.25.Mq, 52.27.Cm, 05.30.Fk, 71.45.Gm}
\keywords{Plasma physics; Multicomponent plasmas; Electron gas; Dielectric function; Dynamic structure factor; Random phase approximation; Linear response; Kramers--Kr\"onig relations; Strong coupling; Riemann zeta function}
\maketitle
...
The causality-constrained Lindhard baseline of Sec.~\ref{sec:finite-temperature-Lindhard}
furnishes a Kramers--Kr\"onig--compliant bare susceptibility $\chi^L(\mathbf{q},\omega;\tau)$
on the real frequency axis.
That baseline is necessary but not sufficient for dense plasmas:
at large plasma coupling $\Gamma$ [Eq.~(\ref{plasma-coupling-parameter-Hartree})],
the bare Coulomb bubble resummed by ordinary RPA encodes only the weak-vertex idealization
and can lose a well-defined dielectric zero $\Re\varepsilon=0$ under strong interaction dressing.
We therefore introduce a \emph{scalar} strong-coupling pathway---hereafter Zeta-RPA---that
dresses the interaction ladder while leaving the dissipative Lindhard content untouched.

For a single species with bare interaction $v(q)$, the scalar RPA response and dielectric function read
\begin{equation}
\chi^{\mathrm{RPA}}(q,\omega)
=
\frac{\chi^L(q,\omega)}{1 - v(q)\,\chi^L(q,\omega)}\,,
\qquad
\varepsilon^{\mathrm{RPA}}(q,\omega)
=
1 - v(q)\,\chi^L(q,\omega)\,.
\label{eq:scalar-rpa}
\end{equation}
Zeta-RPA promotes the interaction weight to a deterministic Riemann-zeta factor $W_\zeta$,
\begin{equation}
\chi^{\zeta}(q,\omega)
=
...
\end{equation}
The coefficients $(\alpha,\beta,\gamma,\delta)$ are recorded in Appendix~\ref{sec:plasma-units}.
...
This construction enforces the weak-coupling identity
...
\subsection{Zeta-RPA coupling-weight coefficients}
\label{sec:zeta-weight-coefficients}

The production weight $W_\zeta$ of Sec.~\ref{sec:zeta-rpa} is completely specified by
Eq.~(\ref{eq:zeta-weight}) once the dimensionless coefficients
$(\alpha,\beta,\gamma,\delta)$ are fixed.
Unless otherwise stated, all numerical Zeta-RPA results in this work---including
Fig.~\ref{fig:zeta-rpa-dispersion}---employ the provisional defaults
\begin{equation}
\alpha = 0.05\,,
\qquad
\beta = 1\,,
...
\end{equation}
together with the Laurent evaluation $W_\zeta=f\,\zeta(1+f)$ for $f>0$
and the identity $W_\zeta=1$ at $f=0$.
The plasma coupling $\Gamma$ entering $f$ is that of
Eq.~(\ref{plasma-coupling-parameter-Hartree});
the reduced temperature $\tau$ is that of Eq.~(\ref{reduced-temperature}).
A strong-coupling gate $\Gamma\ge\Gamma_\star$ with $\Gamma_\star=10$
(or $r_s\ge r_s^\star$ with $r_s^\star=5$) marks the thermodynamic window
in which the zeta dress is active on each channel;
outside that window the undressed RPA is recovered by the $W_\zeta^{st}\to 1$ identity
without additional parameters.
In the two-component production pathway of Sec.~\ref{sec:zeta-rpa-matrix},
diagonal channels use species-local $(\Gamma_e,\Gamma_i)$ while the cross channel
uses the arithmetic mean $\Gamma_{ei}$ of Eq.~(\ref{eq:zeta-cross-regime}).
...
\begin{itemize}
    ...
    \item \textbf{FIG. 9.} Static structure factors: Fig.~\ref{fig:Sq-gamma-ee} ($S_{ee}(\bar{q})$), Fig.~\ref{fig:Sq-gamma-ii} ($S_{ii}(\bar{q})$), and Fig.~\ref{fig:Sq-gamma-ei} ($S_{ei}(\bar{q})$) obtained via direct frequency integration of the dynamic MosaiQ-Lindhard response for a dense plasma at $r_s = 1.0$. In each panel, the temperature is deterministically mapped to sweep the coupling parameter $\Gamma \in \{10, 50, 100, 150\}$, with color denoting the coupling strength and linestyles progressing from dot-dot-dashed ($\Gamma = 10$) and dot-dashed ($\Gamma = 50$) through dashed ($\Gamma = 100$) to solid ($\Gamma = 150$). The main panels showcase the monotonic asymptotic convergence to the ideal-gas limit ($S \to 1$ or $0$) up to the macroscopic horizon $\bar{q} = 50.0$. High-resolution insets provide anatomical magnification of the microscopic sector: Fig.~\ref{fig:Sq-gamma-ee} zooms into $\bar{q} \in [1.4, 2.0]$ to resolve the sharpening of the first $S_{ee}$ correlation peak near $\bar{q} \approx 1.7$, while Figs.~\ref{fig:Sq-gamma-ii} and~\ref{fig:Sq-gamma-ei} magnify $\bar{q} \le 4.0$ to capture the long-wavelength ion--ion structure and the induction of interspecies screening in $S_{ei}$.

    \item \textbf{FIG. 10.} Scalar Zeta-RPA plasmon rescue at strong coupling ($\Gamma = 100$, $r_s = 2.0$, $T = 10\,000\,\mathrm{K}$), constructed from the deterministic dielectric roots of Sec.~\ref{sec:zeta-rpa}.
    Fig.~\ref{fig:zeta-rpa-dispersion}(a) overlays the classical Bohm--Gross long-wavelength anchor~\cite{Bohm-Gross1949} (gray dashed), the scalar RPA dispersion $\bar{\omega}_p^{\mathrm{RPA}}(\bar{q})$ from $\Re\varepsilon^{\mathrm{RPA}}=0$ (crimson), and the Zeta-RPA branch $\bar{\omega}_p^{\zeta}(\bar{q})$ from $\Re\varepsilon^{\zeta}=0$ (blue) on the reduced wavevector window $\bar{q}\le 4$.
    Open markers and the shaded continuum region mark wave vectors where the scalar RPA root bracket collapses while the zeta-dressed dielectric retains a continuous pole trajectory.
    Fig.~\ref{fig:zeta-rpa-dispersion}(b) reports the corresponding on-pole Landau damping $|\Im\varepsilon|$ for the RPA and Zeta pathways, demonstrating that the interaction dress of Eq.~(\ref{eq:zeta-weight}) shifts both the real dispersion and the dissipation spectrum without altering the causal Lindhard input $\chi^L$.
\end{itemize}
