...
Fig.~\ref{fig:thermal-anatomy-c} overlays $T=70\,\mathrm{K}$, $1\,000\,\mathrm{K}$, and $10\,000\,\mathrm{K}$ at $\bar{q}=1.0$: at this coupling the three finite-$T$ curves are thermodynamically coincident and are rendered with distinct linestyles and staggered markers ($\circ$, $\square$, $\triangle$) so their overlap remains legible, isolating the physical residual sharpening visible only against the strict $T=0$ analytic real susceptibility (solid black).
...
Rather, it is the profound manifestation of a topological disconnect---which we term the \emph{reverse Dedekind cut}---superimposed on the ensemble inconsistency diagnosed in Sec.~\ref{sec:finite-temperature-Lindhard}.
...
\begin{itemize}
    ...
    Sweeping the metallic density $r_s \in [1, 5]$ from room temperature down to extreme cryogenic temperatures ($T \approx 10^{-2}\,\mathrm{K}$) typically yields ubiquitous \texttt{NaN}/\texttt{Inf} failures in IEEE~754 double arithmetic near the Stratonovich anchor.
