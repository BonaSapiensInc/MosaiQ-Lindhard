...
\begin{itemize}
    ...
    Panels \textbf{(a)}--\textbf{(b)} show the real and imaginary parts for the electron channel, panels \textbf{(c)}--\textbf{(d)} for the ion channel (kinematically scaled to $\bar{q} \le 50.0$), and panels \textbf{(e)}--\textbf{(f)} for the cross-species channel.
    ...
    Panels~\textbf{(a)}, \textbf{(b)}, and~\textbf{(c)} show $S_{ee}$, $S_{ii}$, and $S_{ei}$, respectively, as logarithmic contour maps of $|S_{st}(\bar{q}, \bar{\omega})|$ on the causal $(\bar{q}, \bar{\omega})$ mesh ($\bar{\omega} \in [0.01, 3.0]$ in electron Fermi units).
    Panels~\textbf{(a)} and~\textbf{(c)} cover $\bar{q} \in [0.1, 4.0]$; panel~\textbf{(b)} extends the ion--ion channel to $\bar{q} = 50.0$.
