...
on a dual-scale $(\bar{q}, \bar{\omega})$ mesh:
focused low-momentum panels use $\bar{q} \in [0.1, 4.0]$ with $\bar{\omega} \in [0.01, 3.0]$ in electron Fermi units,
while the companion wide-range panels extend to $\bar{q} = 50.0$ with an adaptive $\bar{\omega}$ ceiling tracking the non-interacting parabola $\bar{\omega} \approx \bar{q}^2$.

Figs.~\ref{fig:S_ee}--\ref{fig:S_ei_wide} summarize the three diagonal and cross-channel structure factors
$S_{ee}$, $S_{ii}$, and $S_{ei}$ obtained from this run, with each channel shown as a focused low-momentum contour map paired with an extended macro-scale contour on the same causal evaluation pipeline.
...
First, $S_{ee}(\bar{q}, \bar{\omega})$ [Figs.~\ref{fig:S_ee} and~\ref{fig:S_ee_wide}] exhibits a sharp plasmon peak in the high-frequency, high-momentum sector.
...
Second, $S_{ii}(\bar{q}, \bar{\omega})$ [Figs.~\ref{fig:S_ii} and~\ref{fig:S_ii_wide}] is confined to the extreme low-frequency band ($\bar{\omega} \rightarrow 0$),
...
Third, $S_{ei}(\bar{q}, \bar{\omega})$ [Figs.~\ref{fig:S_ei} and~\ref{fig:S_ei_wide}] provides the decisive smoking gun for intercomponent screening and entanglement.
...
Furthermore, the extreme dynamic range of these spectral manifolds---as rendered in the dual-scale logarithmic contour isolines of Figs.~\ref{fig:S_ee}--\ref{fig:S_ei_wide}---serves as a visual testament to the numerical exactness of the MosaiQ-Lindhard pipeline. The focused panels resolve microscopic collective modes and the onset of Landau damping, while the extended panels trace the macroscopic migration of spectral weight toward the non-interacting single-particle parabola $\bar{\omega} \approx \bar{q}^2$ up to $\bar{q}=50.0$. Evaluations based on truncated Matsubara series or finite-difference limits within the standard contour formulations of Refs.~\cite{Giuliani-Vignale2005,Gouedard-Deutsch1978} typically pollute the deep-vacuum regions of the $(\bar{q}, \bar{\omega})$ plane with spurious numerical ringing or diverging artifacts. In contrast, the pristine topological gradients and absolute zero-noise floors observed here confirm that the sinc-quadrature Hilbert transform completely suppresses non-causal leakage, preserving the precise algebraic boundaries of the collective and single-particle continuous spectra.
...
\begin{itemize}
    ...
    \item \textbf{FIG. 3.} Dynamic structure factors from the MosaiQ-Lindhard deterministic sandbox at $r_s = 1.0$ and $T = 10{,}000\,\mathrm{K}$.
    Panels~\textbf{(a)}, \textbf{(c)}, and~\textbf{(e)} are focused low-momentum contour plots ($\bar{q} \le 4.0$) illustrating microscopic collective modes and the onset of Landau damping.
    Panels~\textbf{(b)}, \textbf{(d)}, and~\textbf{(f)} are extended wide-range contour plots up to $\bar{q}=50.0$, capturing the macroscopic asymptotic migration of spectral weight toward the non-interacting single-particle parabola $\bar{\omega} \approx \bar{q}^2$.
    Each contour maps $|S_{st}|$ on a base-10 logarithmic scale from $S_{\min}=\max(S_{>0},10^{-12})$ to $S_{\max}$ (the channel maximum on the displayed mesh).
    The filled color field uses $N_{\mathrm{fill}}=200$ log-uniform levels and the white isolines $N_{\mathrm{line}}=72$ log-uniform contours, $S_k=S_{\min}(S_{\max}/S_{\min})^{k/N}$, so that successive isolines are equally spaced in $\log_{10} S$; focused and extended panels share the same colormap convention but independent dynamic-range normalization.

    \item \textbf{FIG. 4.} Extracted electron plasmon dispersion trajectory and corresponding Landau damping for a partially degenerate plasma at $r_s = 2.0$ and $T = 10{,}000\,\mathrm{K}$.
...
\panelpagefigure[fig:structure-factors]{../output/S_ee_contour.pdf}{Fig.~3(a)}{fig:S_ee}
\panelpagefigure{../output/S_ee_wide_contour.pdf}{Fig.~3(b)}{fig:S_ee_wide}
\panelpagefigure{../output/S_ii_contour.pdf}{Fig.~3(c)}{fig:S_ii}
\panelpagefigure{../output/S_ii_wide_contour.pdf}{Fig.~3(d)}{fig:S_ii_wide}
\panelpagefigure{../output/S_ei_contour.pdf}{Fig.~3(e)}{fig:S_ei}
\panelpagefigure{../output/S_ei_wide_contour.pdf}{Fig.~3(f)}{fig:S_ei_wide}
...
\vfill
\figurepaneltag{Fig.~4}
\registerfigurelabels{fig:plasmon-dispersion}
\end{figure}

\panelpagefigure[fig:t0-limit-validation]{../output/t0_analytic_re_contour.pdf}{Fig.~5(a)}{fig:t0-re-2d}
\panelpagefigure{../output/t0_analytic_im_contour.pdf}{Fig.~5(b)}{fig:t0-im-2d}
\panelpagefigure{../output/t0_limit_validation.pdf}{Fig.~5(c)}{fig:t0-1d-slice}

\panelpagefigure[fig:Sq-gamma]{../output/Sq_gamma_ee.pdf}{Fig.~6(a)}{fig:Sq-gamma-ee}
\panelpagefigure{../output/Sq_gamma_ii.pdf}{Fig.~6(b)}{fig:Sq-gamma-ii}
\panelpagefigure{../output/Sq_gamma_ei.pdf}{Fig.~6(c)}{fig:Sq-gamma-ei}
