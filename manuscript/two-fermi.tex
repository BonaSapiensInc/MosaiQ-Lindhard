...
The first approach is inspired from the Holstein polaron model~\cite{Holstein1959}:
one may find out a new particle or field which describes the physical properties of both components simultaneously.
The second approach is to incorporate the nonequilibrium framework~\cite{Kadanoff-Baym1962,Stefanucci-Leeuwen2013},
but one has to be careful for the case of breakdown of thermalization~\cite{Stefanucci-Leeuwen2013}.
Since both approaches are out of the scope of this project,
we will keep our discussion within the linear response theory.
...
\end{eqnarray*}
The position of these poles presented schematically in Fig.~\ref{contour} 
...
There is an alternative way to evaluate the real part susceptibility by employing the Kramers-Kr\"onig relation,
\begin{equation}
...
The parametric integration method suggested by Gouedard and Deutsch~\cite{Gouedard-Deutsch1978}, as expressed in Eq.~(\ref{summation-form-real-Lindhard}), contains a fatal analytic defect. The visual representation of the simple poles in Fig.~\ref{contour} reveals that they align along the diagonals---specifically, the asymptotes of a rectangular hyperbola $x^2 - y^2 = \gamma$. While this geometric distribution of poles is mathematically correct for the Fermi-Dirac distribution, it physically invalidates the contour integration attempted in Eq.~(\ref{differential-Lindhard-analytic-continuation}).
...
The primary breakdown occurs due to the violation of Jordan's Lemma on the infinite semicircular arc in the upper-half complex plane. If we set $\kappa = R e^{i\theta}$, the behavior of the Fermi-Dirac distribution $n(\kappa)$ is governed by the factor $\cos 2\theta$ in its exponent. Outside the diagonals ($\theta < \pi/4$ or $\theta > 3\pi/4$), $\cos 2\theta > 0$, yielding $n(\kappa) \rightarrow 0$. However, in the vast region strictly between the diagonals ($\pi/4 < \theta < 3\pi/4$), $\cos 2\theta < 0$, which causes $e^{(\kappa^2-\gamma)/\tau} \rightarrow 0$. Consequently, the distribution function stubbornly survives as $n(\kappa) \rightarrow 1$. Because the integral over the infinite arc does not vanish, the fundamental premise of applying the residue theorem is utterly destroyed.

Furthermore, an illegal interchange of the order of the frequency integration $\int d\bar{\omega}^\prime$ and the infinite pole summation $\sum_{j=0}^{\infty}$ (a severe violation of Fubini's theorem) is committed. As $j \rightarrow \infty$, the distance between adjacent poles shrinks as $\Delta |\kappa| \sim \mathcal{O}(j^{-1/2})$, forming an infinitely dense ``wall of singularities.'' When the integration is forcibly performed prior to the summation, the wrapped terms scale as $\kappa_j^{\prime\prime} / (-|\kappa_j|^2) \sim -j^{-1/2}$. By the $p$-series test, the summation $\sum j^{-1/2}$ diverges unconditionally to infinity:
\begin{equation}
\left[ \tan^{-1}\left( \frac{\xi_+ + \kappa_j^\prime}{\kappa_j^{\prime\prime}} \right) + \tan^{-1}\left( \frac{\xi_+ - \kappa_j^\prime}{\kappa_j^{\prime\prime}} \right) - \tan^{-1}\left( \frac{\xi_- + \kappa_j^\prime}{\kappa_j^{\prime\prime}} \right) - \tan^{-1}\left( \frac{\xi_- - \kappa_j^\prime}{\kappa_j^{\prime\prime}} \right) \right] \sim \mathcal{O}\left(j^{-1/2}\right).
\end{equation}

Thus, Eq.~(\ref{summation-form-real-Lindhard}) does not converge at all and fails to represent any physical response function due to this breakdown of uniform convergence. This topological defect confirms that bypassing the Matsubara poles via the Kramers-Kr\"onig relation and exact numerical sinc-quadrature (as strictly formalized in Sec.~\ref{sec:finite-temperature-Lindhard}) is not an option, but the only rigorous deterministic pathway to evaluating the finite-temperature Lindhard function.

\section{An alternative way of evaluation of the finite temperature Lindhard function}
\label{sec:Kim-Lee}

The hybrid evaluation method proposed by Kim and Lee~\cite{Kim-Lee1998}, which introduces a direct summation up to a cutoff $m$ combined with a truncated Euler-Maclaurin expansion, attempts to regularize the slow convergence of the Khanna-Glyde formalism. However, numerical implementations reveal that this approach fails to deliver physically correct features across the entire $\bar{q}$-$\bar{\omega}$ domain.

The root cause of this breakdown is identical to the topological defect exposed in Appendix~\ref{sec:Gouedard-Deutsch}. Because the underlying analytical continuation forces the tail of the Matsubara series to diverge as $\mathcal{O}(j^{-1/2})$, applying the Euler-Maclaurin formula to an inherently divergent series merely amplifies artificial numeric aliasing and unphysical fluctuations. It does not possess a mathematically sound regularization anchor.

Consequently, to achieve absolute deterministic control over the non-equilibrium inference trajectory, one must entirely bypass the unstable infinite summation over Matsubara poles. The only mathematically rigorous and physically unyielding solution is the direct numerical evaluation of the Kramers-Kr\"onig relation via the Hilbert transformation Eq.~(\ref{real-Lindhard-Kramers-Kronig}) developed in Sec.~\ref{sec:finite-temperature-Lindhard}. By pinning the real part directly to the exact dissipation of the imaginary part using the sinc-quadrature rule Eq.~(\ref{sinc-quadrature-Cauchy-principal-value-integral}), the calculation guarantees an error bound of order $10^{-259}$, bypassing the bottomless pit of series divergence and establishing a true deterministic control sandbox.
...
\bibitem{Kadanoff-Baym1962} L. P. Kadanoff and G. Baym, \textit{Quantum Statistical Mechanics} (W. A. Benjamin, Inc., New York, 1962).

\bibitem{Stefanucci-Leeuwen2013} G. Stefanucci and R.  van Leeuwen,
