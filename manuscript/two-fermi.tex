...
% Register \label anchors for submission figures without printing a \caption block.
\newcommand{\registerfigurelabels}[2][]{%
...
% Optional first argument: additional cross-reference label (e.g., composite figure).
\newcommand{\panelpagefigure}[4][]{%
  \clearpage
  \begin{figure}[p]
    \centering
    \vfill
...
on a dense simulator mesh with $\bar{q} \in [0.1, 50.0]$ and $\bar{\omega} \in [0.01, 3.0]$ in electron Fermi units.
The contour figures below resample these causal grids onto channel-specific display rectangles with integer axis limits matched to the $T=0$ reference panels of Fig.~\ref{fig:t0-limit-validation}.
...
Panels~\textbf{(a)}--\textbf{(b)} map $|\Re \chi_{e}^L|$ and $|\Im \chi_{e}^L|$ on the plasmon-sector window $\bar{q} \in [0, 4.0]$ and $\bar{\omega} \in [0, 3.0]$;
panels~\textbf{(e)}--\textbf{(f)} show the corresponding bare cross-coupling products $|\Re \chi_{ei}^L|$ and $|\Im \chi_{ei}^L|$ on the same window.
Rendered with a unified high-contrast base-10 logarithmic color scale ($N_{\mathrm{fill}}=200$ filled levels, $N_{\mathrm{line}}=72$ white isolines equally spaced in $\log_{10}|\chi^L|$), these panels expose the raw single-particle continuum and Landau boundaries of the light electron fluid.

Due to the extreme mass asymmetry ($m_i/m_e \gg 1$), the ion manifolds in panels~\textbf{(c)}--\textbf{(d)} require a kinematic Golden Scale: an expanded momentum horizon $\bar{q} \in [0, 50.0]$ together with an extreme low-frequency zoom $\bar{\omega} \in [0, 0.20]$.
Without this $15\times$ frequency magnification, the ion-acoustic weight would collapse into an unreadable strip at the bottom of the electron-scale $\bar{\omega}$ axis; the zoom restores direct visual access to the ion single-particle continuum and acoustic damping profile while preserving the high-$\bar{q}$ tail up to the macroscopic horizon.
...
Panels~\textbf{(a)} and~\textbf{(c)} display $|S_{ee}|$ and $|S_{ei}|$ on $\bar{q} \in [0, 4.0]$ and $\bar{\omega} \in [0, 3.0]$;
panel~\textbf{(b)} applies the same ion Golden Scale ($\bar{q} \in [0, 50.0]$, $\bar{\omega} \in [0, 0.20]$) to $|S_{ii}|$, framing the low-frequency ion-acoustic collective ridge.
...
To quantitatively isolate these mechanisms, we map the evolution of the bare response under a thermal sharpening ladder in Fig.~\ref{fig:thermal-anatomy}.
As the temperature scales down ($T \rightarrow 11\,\mathrm{K}$), the single-particle continuum boundaries continuously sharpen onto the analytic $T=0$ topology with no mesh-dependent discontinuity.
...
First, $S_{ee}(\bar{q}, \bar{\omega})$ [Fig.~\ref{fig:S_ee}] exhibits a sharp plasmon peak in the high-frequency, high-momentum sector.
...
Second, $S_{ii}(\bar{q}, \bar{\omega})$ [Fig.~\ref{fig:S_ii}] is confined to the extreme low-frequency band ($\bar{\omega} \lesssim 0.2$ in electron Fermi units),
...
and the structure factor weight concentrates in the acoustic window rather than tracking the electron plasmon dispersion; the Golden Scale of Fig.~\ref{fig:S_ii} ($\bar{q}$ to $50.0$, $\bar{\omega}$ to $0.20$) makes this manifold fully visible rather than compressed against the $\bar{\omega}=0$ axis.

Third, $S_{ei}(\bar{q}, \bar{\omega})$ [Fig.~\ref{fig:S_ei}] provides the decisive smoking gun for intercomponent screening and entanglement.
...
Standard Matsubara/contour implementations that evaluate $\Re \chi^L$ through divergent pole summations~\cite{Gouedard-Deutsch1978,Khanna-Glyde1976},
...
In Fig.~\ref{fig:Sq-gamma}(a), the inset spanning $\bar{q} \in [1.4, 2.0]$ magnifies the $S_{ee}$ peak at $\bar{q} \approx 1.7$ and reveals the progressive heightening of the first coordination shell as $\Gamma$ increases, a hallmark of nascent short-range order.
Similarly, the insets in Fig.~\ref{fig:Sq-gamma}(b) and (c) capture the precise long-wavelength behavior of the ion--ion and electron--ion correlations for $\bar{q} \le 4.0$.
The induction of a distinct peak in $S_{ei}$ [Fig.~\ref{fig:Sq-gamma}(c) inset], despite its small magnitude ($\mathcal{O}(10^{-4})$), quantifies the exact electron-drag effect where the light electron fluid adheres to the heavy-ion acoustic manifold.
...
\subsection{Hilbert-transform reconstruction on the real axis}
...
\textbf{Methodological contrast with standard Matsubara/contour formulations.}
...
\begin{itemize}
    ...
    \item \textbf{FIG. 3.} The non-interacting base manifolds of the finite-temperature Lindhard response functions $\chi_{st}^L(\bar{q}, \bar{\omega})$ at $r_s = 1.0$ and $T = 10{,}000\,\mathrm{K}$, resampled onto channel-specific display rectangles with integer axis limits.
    Panels~\textbf{(a)}--\textbf{(b)} show $|\Re \chi_{e}^L|$ and $|\Im \chi_{e}^L|$ on $\bar{q} \in [0, 4.0]$ and $\bar{\omega} \in [0, 3.0]$.
    Panels~\textbf{(c)}--\textbf{(d)} show $|\Re \chi_{i}^L|$ and $|\Im \chi_{i}^L|$ on the ion Golden Scale $\bar{q} \in [0, 50.0]$ and $\bar{\omega} \in [0, 0.20]$, magnifying the acoustic band that would otherwise be compressed against the origin on the electron frequency axis.
    Panels~\textbf{(e)}--\textbf{(f)} show $|\Re \chi_{ei}^L|$ and $|\Im \chi_{ei}^L|$ on the cross-channel window $\bar{q} \in [0, 4.0]$ and $\bar{\omega} \in [0, 3.0]$, with bold isolines for legibility.
    ...
    Panels~\textbf{(a)} and~\textbf{(c)} show $|S_{ee}|$ and $|S_{ei}|$ on $\bar{q} \in [0, 4.0]$ and $\bar{\omega} \in [0, 3.0]$ in electron Fermi units.
    Panel~\textbf{(b)} shows $|S_{ii}|$ on the ion Golden Scale $\bar{q} \in [0, 50.0]$ and $\bar{\omega} \in [0, 0.20]$, isolating the low-frequency ion-acoustic collective mode.
    ...
    The filled color field uses $N_{\mathrm{fill}}=200$ log-uniform levels and white isolines $N_{\mathrm{line}}=72$ log-uniform contours, $S_k=S_{\min}(S_{\max}/S_{\min})^{k/N}$, so that successive isolines are equally spaced in $\log_{10} S$; panel~\textbf{(c)} uses the bold cross-channel isoline profile of Fig.~\ref{fig:bare-lindhard}(f).
    ...
    \textbf{(a)} The extracted dispersion trajectory (solid curve) and the classical Bohm--Gross limit~\cite{Bohm-Gross1949} (dashed curve) superimposed directly on top of a 2D background contour map of the canonical spectral response $S_{ee}(\bar{q}, \bar{\omega})$ (filled color field in the legend).
    The rainbow background encodes $|S_{ee}|$ on a base-10 logarithmic scale; the solid black curve is the extracted plasmon branch $\bar{\omega}_p(\bar{q})$, and the dashed curve is the Bohm--Gross long-wavelength anchor.
    \textbf{(b)} The exact multi-component Landau damping rate $\Im[\varepsilon(\bar{q}, \bar{\omega}_p)]$ (solid curve) plotted against the non-interacting single-particle excitation (SPE) continuum baseline, illustrating the sudden dissipation onset at the kinematic boundary.
    ...
    \item \textbf{FIG. 6.} Highly degenerate limit validation at extreme degeneracy ($\tau = 10^{-4}$, $T \approx 11\,\mathrm{K}$). \textbf{(a)--(b)} The causality-pinned $T = 0$ Lindhard susceptibility topology over the $(\bar{q}, \bar{\omega})$ plane, derived via the Kramers--Kr\"onig reconstruction of Appendix~\ref{sec:Lindhard-function}, rendered as base-10 logarithmic contour maps of $|\Re \tilde{\chi}^L_{T=0}|$ and $|\Im \tilde{\chi}^L_{T=0}|$ with the same white isoline convention as Fig.~\ref{fig:bare-lindhard}. A black dashed vertical guide marks the $\bar{q} = 1.0$ slice used in panel~\textbf{(c)}. \textbf{(c)} Top panel: 1D extraction along $\bar{q} = 1.0$ comparing the standard contour-based reference of Refs.~\cite{Giuliani-Vignale2005,Gouedard-Deutsch1978} (solid curves) against the MosaiQ-Lindhard engine (markers). Bottom panel: absolute errors $|\Delta \Re|$ and $|\Delta \Im|$ on a logarithmic scale. While $\Im \chi^L$ flawlessly converges to the $\tau \rightarrow 0$ limit, $\Re \chi^L$ retains an $\mathcal{O}(1)$ offset, visually capturing the ensemble inconsistency of the standard Matsubara/contour formulations discussed in Sec.~\ref{sec:finite-temperature-Lindhard}.

    \item \textbf{FIG. 7.} The anatomy of thermal blurring for the electron susceptibility $\chi_e^L(\bar{q}, \bar{\omega})$ at $r_s = 1.0$.
    Panels~\textbf{(a)}--\textbf{(b)} exhibit the thermal sharpening ladder for $|\Im\chi_e^L|$ at intermediate ($T=1{,}000\,\mathrm{K}$) and highly degenerate ($T=11\,\mathrm{K}$) temperatures, showing smooth topological convergence to the exact $T=0$ limit.
    Panel~\textbf{(c)} displays a high-resolution 1D cross-section at $\bar{q}=1.0$ overlaying $T=11\,\mathrm{K}$, $1{,}000\,\mathrm{K}$, and $10{,}000\,\mathrm{K}$ against the strict $T=0$ step boundaries to isolate textbook Fermi-step broadening.
    Panel~\textbf{(d)} maps the bare-vs-RPA interaction contrast ratio, demonstrating that diffuse weight is restricted to the bare baseline.

    \item \textbf{FIG. 8.} Static structure factors \textbf{(a)} $S_{ee}(\bar{q})$, \textbf{(b)} $S_{ii}(\bar{q})$, and \textbf{(c)} $S_{ei}(\bar{q})$ obtained via direct frequency integration of the dynamic MosaiQ-Lindhard response for a dense plasma at $r_s = 1.0$. In each panel, the temperature is deterministically mapped to sweep the coupling parameter $\Gamma \in \{10, 50, 100, 150\}$. The main panels showcase the monotonic asymptotic convergence to the ideal-gas limit ($S \to 1$ or $0$) up to the macroscopic horizon $\bar{q} = 50.0$. High-resolution insets provide anatomical magnification of the microscopic sector: panel \textbf{(a)} zooms into $\bar{q} \in [1.4, 2.0]$ to resolve the sharpening of the first $S_{ee}$ correlation peak near $\bar{q} \approx 1.7$, while panels \textbf{(b)}--\textbf{(c)} magnify $\bar{q} \le 4.0$ to capture the long-wavelength ion--ion structure and the induction of interspecies screening in $S_{ei}$.
\end{itemize}
...
\vfill
\figurepaneltag{Fig.~1}
\registerfigurelabels{contour}
\end{figure}
...
\vfill
\figurepaneltag{Fig.~2}
\registerfigurelabels{Fermi-Dirac-contour-integral}
\end{figure}

\panelpagefigure[fig:bare-lindhard]{../output/chi_e_re.pdf}{Fig.~3(a)}{fig:chi_e_re}
\panelpagefigure{../output/chi_e_im.pdf}{Fig.~3(b)}{fig:chi_e_im}
\panelpagefigure{../output/chi_i_re.pdf}{Fig.~3(c)}{fig:chi_i_re}
\panelpagefigure{../output/chi_i_im.pdf}{Fig.~3(d)}{fig:chi_i_im}
\panelpagefigure{../output/chi_ei_re.pdf}{Fig.~3(e)}{fig:chi_ei_re}
\panelpagefigure{../output/chi_ei_im.pdf}{Fig.~3(f)}{fig:chi_ei_im}

\panelpagefigure[fig:structure-factors]{../output/S_ee_contour.pdf}{Fig.~4(a)}{fig:S_ee}
\panelpagefigure{../output/S_ii_contour.pdf}{Fig.~4(b)}{fig:S_ii}
\panelpagefigure{../output/S_ei_contour.pdf}{Fig.~4(c)}{fig:S_ei}
...
\vfill
\figurepaneltag{Fig.~5}
\registerfigurelabels{fig:plasmon-dispersion}
\end{figure}

\panelpagefigure[fig:t0-limit-validation]{../output/t0_analytic_re_contour.pdf}{Fig.~6(a)}{fig:t0-re-2d}
\panelpagefigure{../output/t0_analytic_im_contour.pdf}{Fig.~6(b)}{fig:t0-im-2d}
\panelpagefigure{../output/t0_limit_validation.pdf}{Fig.~6(c)}{fig:t0-1d-slice}

\clearpage
\begin{figure}[p]
\centering
\vfill
\includegraphics[width=\textwidth]{../output/thermal_anatomy.pdf}
\vfill
\figurepaneltag{Fig.~7}
\registerfigurelabels{fig:thermal-anatomy}
\end{figure}

\panelpagefigure[fig:Sq-gamma]{../output/Sq_gamma_ee.pdf}{Fig.~8(a)}{fig:Sq-gamma-ee}
\panelpagefigure{../output/Sq_gamma_ii.pdf}{Fig.~8(b)}{fig:Sq-gamma-ii}
\panelpagefigure{../output/Sq_gamma_ei.pdf}{Fig.~8(c)}{fig:Sq-gamma-ei}
