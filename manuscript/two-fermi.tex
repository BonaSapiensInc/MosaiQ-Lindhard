...
\end{subequations}
Note that we suppressed the effects of spin-polarizations of each fermion components.
...
\end{equation}
where $A_1$ is a certain constant depending on $d$ and $\alpha$.
...
the choice ensures a theoretical truncation error bound of $\mathcal{O}(10^{-259})$, which securely anchors the evaluation well below double-precision machine limits and eliminates numerical instabilities from the poles of $f\left( x \right)$.
...
of those modes consist most parts of many-body 
...
we executed the SOPHUS-Lindhard C++20 sandbox on a partially degenerate two-component Fermion plasma
...
at a theoretical truncation error bound of $\mathcal{O}(10^{-259})$, thereby injecting spurious numerical noise
...
We subjected the SOPHUS-Lindhard engine to an automated validation of all four relations
...
A complementary stress test of the SOPHUS-Lindhard pipeline is the absolute zero-temperature limit
...
This demonstrates that the SOPHUS route remains stable when the thermal broadening of the
...
Figure~\ref{fig:t0-limit-validation} displays this behavior at $\bar{q} = 1$:
the engine output (markers) tracks a well-defined, repeatable trajectory,
while the Appendix~\ref{sec:Lindhard-function} reference (solid curves) occupies a distinct manifold in the complex plane.
The bottom panel shows that the absolute errors in $\Re \chi^L$ remain at the $\mathcal{O}(1)$ level,
whereas the imaginary errors are confined to the truncation floor of the sinc evaluation.

This offset must not be interpreted as a defect of the numerical engine.
It is the operational signature of the ensemble inconsistency diagnosed in Sec.~\ref{sec:finite-temperature-Lindhard}:
legacy $T = 0$ analytical formulas assemble $\Re \chi$ and $\Im \chi$ from thermodynamically distinct prescriptions
---canonical contour sums or Giuliani--Vignale zero-temperature limits for the real part,
grand-canonical dissipative kernels for the imaginary part---
that are not jointly constrained by a single Kramers--Kr\"onig relation.
The SOPHUS engine, by contrast, evaluates $\Re \chi^L$ exclusively as the Hilbert transform
...
at a theoretical truncation error bound of $\mathcal{O}(10^{-259})$, completely bypassing the infinite Matsubara pole barrier by shifting the integration domain into a strictly constrained physical space in which
...
We therefore conclude that the SOPHUS-Lindhard framework provides an absolute deterministic sandbox
...
The deterministic C++20 engine (SOPHUS-Lindhard) and the associated Python visualization scripts used to generate the results in this study are open-source and publicly available. To facilitate independent verification of the macroscopic sum rules, the deterministic plasmon dispersion extraction, and the mathematical breakdown of the legacy contour-integration method at the highly degenerate $T \to 0$ limit, readers are strongly encouraged to clone the repository and reproduce the findings. The complete source code, test suites, and build instructions can be accessed at: \url{https://github.com/YOUR_GITHUB_USERNAME/SOPHUS-Lindhard}.
...
\end{eqnarray*}
where in the last equality we used the expression of free fermion density of state, 
...
required by Fubini's theorem---yields pole contributions scaling as
\begin{equation}
\left[
\tan^{-1}\!\left( \frac{\xi_+ + \kappa_j^\prime}{\kappa_j^{\prime\prime}} \right)
+ \tan^{-1}\!\left( \frac{\xi_+ - \kappa_j^\prime}{\kappa_j^{\prime\prime}} \right)
...
and the notifying the fact that $\Gamma\left( \frac{1}{2} \right) = \sqrt{\pi}$,
we have
\begin{equation}
\int_0^\infty d\kappa \, n\left( \kappa \right)
...
the real part is obtained with a theoretical truncation error bound of $\mathcal{O}(10^{-259})$
...
\begin{subfigure}[t]{0.48\textwidth}
  \centering
  \includegraphics[width=\linewidth]{../output/S_ee_contour.png}
...
\begin{subfigure}[t]{0.48\textwidth}
  \centering
  \includegraphics[width=\linewidth]{../output/S_ee_3d.png}
...
\begin{subfigure}[t]{0.48\textwidth}
  \centering
  \includegraphics[width=\linewidth]{../output/S_ii_contour.png}
...
\begin{subfigure}[t]{0.48\textwidth}
  \centering
  \includegraphics[width=\linewidth]{../output/S_ii_3d.png}
...
\begin{subfigure}[t]{0.48\textwidth}
  \centering
  \includegraphics[width=\linewidth]{../output/S_ei_contour.png}
...
\begin{subfigure}[t]{0.48\textwidth}
  \centering
  \includegraphics[width=\linewidth]{../output/S_ei_3d.png}
  \caption{$S_{ei}$ surface.}
  \label{fig:S_ei_3d}
\end{subfigure}

\caption{Dynamic structure factors from the SOPHUS-Lindhard deterministic sandbox at $r_s = 1.0$ and $T = 10{,}000\,\mathrm{K}$.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.72\textwidth]{../output/t0_limit_validation.pdf}
\caption{Highly degenerate limit validation at $\bar{q} = 1.0$, $\gamma = 1$, and $\tau = 10^{-4}$
($T \approx 11\,\mathrm{K}$ in electron Fermi units).
\textbf{Top:} exact $T = 0$ Lindhard susceptibility from Appendix~\ref{sec:Lindhard-function} (solid curves)
and SOPHUS engine evaluation (markers) for $\Re \tilde{\chi}$ and $\Im \tilde{\chi}$
in units of $\hbar D(\epsilon_\mathrm{F})$.
\textbf{Bottom:} absolute errors $|\Delta \Re|$ and $|\Delta \Im|$ on a logarithmic scale,
showing that $\Im \chi^L$ converges to the $\tau \rightarrow 0$ limit of Eq.~(\ref{imaginary-Lindhard})
while $\Re \chi^L$ retains an $\mathcal{O}(1)$ offset from the Appendix~\ref{sec:Lindhard-function} reference,
consistent with the ensemble inconsistency of Sec.~\ref{sec:finite-temperature-Lindhard}.}
