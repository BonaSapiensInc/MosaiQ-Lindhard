...
the scalar RPA dispersion $\omega_p^{\mathrm{RPA}}(q)$, and the Zeta-RPA branch $\omega_p^{\zeta}(q)$
...
\begin{itemize}
    ...
    \item \textbf{FIG. 2.} Scalar Zeta-RPA plasmon rescue at strong coupling ($\Gamma = 100$, $r_s = 2.0$, $T = 10\,000\,\mathrm{K}$), constructed from the deterministic dielectric roots of Sec.~\ref{sec:zeta-rpa}.
    Fig.~\ref{fig:zeta-rpa-dispersion}(a) overlays the classical Bohm--Gross long-wavelength anchor~\cite{Bohm-Gross1949} (gray dashed), the standard scalar RPA dispersion $\bar{\omega}_p^{\mathrm{RPA}}(\bar{q})$ from $\Re\varepsilon^{\mathrm{RPA}}=0$ (crimson), and the Zeta-RPA branch $\bar{\omega}_p^{\zeta}(\bar{q})$ from $\Re\varepsilon^{\zeta}=0$ (blue) on the reduced wavevector window $\bar{q}\le 4$.
    Open markers and the shaded continuum region highlight wave vectors where the standard RPA root bracket collapses, while the zeta-dressed dielectric retains a continuous, finite pole trajectory.
    Fig.~\ref{fig:zeta-rpa-dispersion}(b) reports the corresponding on-pole Landau damping $|\Im\varepsilon|$, demonstrating that the analytic Riemann-zeta continuation of Eq.~(\ref{eq:zeta-weight}) heals the many-body divergence and shifts the dissipation spectrum without altering the underlying Kramers--Kr\"onig-constrained Lindhard input $\chi^L$.
