\let\orilabel\label
\documentclass[aps,showkeys,noshowpacs,superscriptaddress,pre,preprint]{revtex4-2}
\let\label\orilabel
\usepackage{graphicx}
...
\affiliation{Bona Sapiens, Inc., Seoul, Republic of Korea}

\date{\today}

\begin{abstract}
We present a deterministic, causality-constrained formulation for the finite-temperature Lindhard linear response and two-component random-phase approximation (RPA) dielectric functions, resolving analytic inconsistencies in widely used routes based on contour integration over Matsubara frequencies. We demonstrate that pole-summation formulations---which assemble the real susceptibility from infinite residue sums over Matsubara-like poles---are hindered by violations of Jordan's lemma, divergent series tails, and order interchanges not justified by Fubini's theorem; hybrid treatments that combine grand-canonical dissipation with canonical contour sums further induce ensemble inconsistency, compromising exact causality and producing persistent $\mathcal{O}(1)$ real-part offsets from exact $T=0$ analytical limits in the highly degenerate regime. Elevating the Kramers--Kr\"onig relation to the primary construction principle rather than an \emph{a posteriori} consistency check, we derive the imaginary part in closed form within the grand-canonical ensemble and obtain the real part solely as its Hilbert transform on a strictly constrained real-axis physical space, bypassing the infinite Matsubara pole barrier entirely. By employing a modified sinc-quadrature rule with theoretical truncation error bound $\mathcal{O}(\sqrt{N} \exp(-\sqrt{\pi d \alpha N}))$ (where $N$ is the truncation limit, $d$ is the half-width of the analytic strip, and $\alpha$ is a convergence parameter), we achieve evaluation accuracy exponentially suppressed well below machine precision ($\sim 10^{-259}$ for $N=16384$, $d/\alpha=7$). We validate this deterministic framework by extracting the electron plasmon dispersion and exact Landau damping via derivative-free root finding, and by demonstrating precise satisfaction of four macroscopic dynamic structure factor sum rules within machine precision without ad hoc regularization or parameter tuning. Furthermore, we resolve the longstanding $\mathcal{O}(1)$ topological discrepancy in the zero-temperature limit of the Lindhard function. By introducing the concept of a \emph{reverse Dedekind cut}---arising from the intrinsic zero-point avoidance of fermionic Matsubara frequencies---we demonstrate that the macroscopic step function must strictly evaluate to $f(0)=1/2$ at the Fermi level to preserve Kramers--Kr\"onig causality. Guided by this Stratonovich imperative, we propose a singularity excision algorithm that analytically subdues the catastrophic numerical divergence, extending the IEEE~754 double-precision limit of the causal response deep into extreme cryogenic regimes.

Building on this causal Lindhard foundation, we address the catastrophic breakdown of the standard RPA geometric series in the strong-coupling regime through a dual-pathway construction. Pathway~A elevates the RPA ladder to the polylogarithm family $\mathrm{Li}_s(v\chi^L)$ (PolyLog-RPA), recovering ordinary RPA as $s\to 0$ and the Riemann zeta boundary as $v\chi^L\to 1$ for $\Re s>1$. Pathway~B---the production \emph{Zeta-RPA} route used for all numerical results---projects that topological boundary onto a macroscopic local field correction (LFC) skeleton by dressing the bare Coulomb interaction with a global weight $W_\zeta(\Gamma,r_s,\tau)\equiv 1-G_\zeta$, exactly recovering ordinary RPA as $\Gamma\to 0$ while retaining continuous dielectric roots and a modified Landau-damping spectrum at extreme coupling. For two-component plasmas with extreme mass asymmetry, this construction is promoted to an asymmetric zeta matrix with independent diagonal regimes $(\Gamma_e,\Gamma_i)$ and an arithmetic-mean cross channel $\Gamma_{ei}=(\Gamma_e+\Gamma_i)/2$, leaving the bare Lindhard susceptibilities untouched. This regularized multi-component framework completely suppresses non-physical scattering artifacts in the dynamic structure factors, cleanly resolving ion-acoustic resonant pockets and low-frequency electron-drag entanglement. Furthermore, static structure factor evaluations at extreme coupling ($\Gamma \ge 100$) deterministically capture Wigner crystallization precursor peaks and heavy-ion structural oscillations, entirely bypassing the need for phenomenological liquid-state closures.
\end{abstract}
%\pacs{52.25.Mq, 52.27.Cm, 05.30.Fk, 71.45.Gm}
\keywords{Plasma physics; Multicomponent plasmas; Electron gas; Dielectric function; Dynamic structure factor; Static structure factor; Random phase approximation; Zeta-RPA; Local field correction; Polylogarithm; Linear response; Kramers--Kr\"onig relations; Strong coupling; Riemann zeta function; Wigner crystallization; Landau damping}
\maketitle
...
The Lindhard function~\cite{Lindhard1954} is written as~\cite{Giuliani-Vignale2005}
\begin{equation}
...
\subsection{Regularization of the static susceptibility at finite temperature}
\label{sec:finite-T-regularization}

While the zero-temperature ($T=0$) Lindhard function is ubiquitous in standard many-body perturbation theory, its reliance on a discontinuous Fermi surface introduces non-physical artifacts.
Most notably, the sharp step-function nature of the $T=0$ Fermi--Dirac occupations $n_{\mathbf{k}\sigma}$ forces a logarithmic singularity in the derivative of the static susceptibility at $q=2k_\mathrm{F}$ (the Kohn anomaly)~\cite{Kohn1959}.
In conventional computational solid-state physics, such singular behaviors are often masked by introducing anomalously large empirical broadening factors ($\eta$ or $\delta$) or by implicitly truncating the analytical structure via pseudopotential constraints.

We argue that evaluating the dielectric response at finite temperature ($T>0$) is not merely a thermodynamic refinement, but a fundamental requirement for preserving the analytic structure of the response manifold.
At $T>0$, the Fermi--Dirac distribution $n_{\mathbf{k}\sigma}$ [later written in reduced variables as Eq.~(\ref{eq:fermi-kappa})] naturally smooths the discontinuous occupation boundaries that underlie Eq.~(\ref{Lindhard}).
However, the exact numerical integration of the finite-temperature independent-particle polarizability $\chi^L(\mathbf{q},\omega)$ in Eq.~(\ref{Lindhard}) presents a severe computational challenge due to the dense accumulation of closely spaced poles near the real energy axis.
By employing the modified sinc-quadrature method developed below, we resolve the finite-$T$ spectrum without resorting to artificial smearing.

The exact thermal smearing intrinsically heals the $T=0$ divergences and maintains strict causality (Kramers--Kr\"onig compliance) down to the machine-precision floor documented in Secs.~\ref{sec:stratonovich-imperative} and~\ref{sec:singularity-excision}.
Consequently, the discrepancy between the $T=0$ and finite-$T$ formulations represents a qualitative restoration of physical stability rather than a numerical inconvenience.
This robust, divergence-free baseline at finite temperature establishes the analytic foundation required for subsequent strong-coupling and vertex-corrected extensions of the random-phase approximation (RPA).
...
\end{equation*}
\begin{equation}
n_\mathbf{k} \rightarrow n(\kappa) = \left[e^{\left(\kappa^2 - \gamma\right)/\tau} + 1\right]^{-1},
\label{eq:fermi-kappa}
\end{equation}
...
Following the analytic continuation route of Ref.~\cite{Khanna-Glyde1976}, one might formally treat the squared momentum as a generic complex variable, $\theta = \kappa^2$. By invoking Euler's identity, $e^{i\pi} + 1 = 0$, the roots of the Fermi--Dirac denominator effortlessly emerge at $\theta_j = \gamma \pm i\pi\tau_j$. Mapping this back to the momentum space yields the complex squared-momenta of Eq.~(\ref{series-poles}). (As we will demonstrate below and in Appendix~\ref{sec:Gouedard-Deutsch}, however, this very formal maneuver---forcing $\kappa^2$ into the complex plane to enforce a contour closure---is the precise topological origin of the analytic breakdown and the divergent Matsubara-series tails in the standard contour-integration routes of Refs.~\cite{Khanna-Glyde1976,Gouedard-Deutsch1978}. This formal continuation maps the continuous real-momentum integration onto a discrete set of complex poles. Such a mapping introduces subtle questions regarding ensemble consistency, as it effectively decomposes the single continuous grand-canonical ensemble into an unphysical discrete mixture of non-Hermitian states. We refer to this structural characteristic as ensemble fragmentation (see Appendix~\ref{sec:Gouedard-Deutsch} for a detailed topological analysis).)
...
the Fermi--Dirac factor $n(\kappa)$ of Eq.~(\ref{eq:fermi-kappa})
...
However, our rigorous convergence analysis shows that the infinite summation in Eq.~(\ref{formal-real-Lindhard}) exhibits extremely slow convergence characterized by a power-law tail of order $\mathcal{O}(j^{-1/2})$, which warrants meticulous numerical truncation and careful handling rather than unconstrained summation~\cite{Khanna-Glyde1976,Gouedard-Deutsch1978}.
...
These hybrid treatments assign the imaginary and real parts to differing thermodynamic prescriptions, which, as our explicit numerical evaluations confirm, leads to persistent deviations from the exact Kramers--Kr\"onig relations ($\Delta \sim \mathcal{O}(1)$ in the real part within the highly degenerate regime), becoming particularly pronounced as $T \rightarrow 0$.
...
from the divergent Matsubara-series tail $\mathcal{O}(j^{-1/2})$,
and from interchanging the frequency integration with the infinite pole sum without the uniform-convergence conditions required by Fubini's theorem~\cite{Rudin1976}.
...
the Cauchy principal value integration formula~\cite{Arfken-Weber1995,Byron-Fuller1992}
\begin{equation*}
...
\section{Analytic continuation of the RPA kernel in the strong-coupling regime}
\label{sec:zeta-rpa}

The causality-constrained Lindhard baseline of Sec.~\ref{sec:finite-temperature-Lindhard} furnishes a Kramers--Kr\"onig--compliant bare susceptibility $\chi^L(\mathbf{q},\omega;\tau)$ on the real frequency axis. That baseline is necessary but not sufficient for dense plasmas. At large plasma coupling $\Gamma$ [Eq.~(\ref{plasma-coupling-parameter-Hartree})], the bare Coulomb bubble resummed by ordinary RPA encounters a catastrophic divergence of the geometric series $\sum (v\chi^L)^n$.

We resolve this breakdown with two parallel frameworks that share the same weak-coupling limit and the same causal Lindhard input, but play distinct theoretical roles (Appendix~\ref{sec:zeta-weight-coefficients}).

\paragraph{Pathway A --- PolyLog-RPA (formal definition).}
The RPA interaction ladder is the $s=0$ member of the polylogarithm family,
$\mathrm{Li}_0(x)=\sum_{n=1}^{\infty}x^n$. Elevating the diagrammatic series by a continuous topological penalty $n^{-s}$ defines the PolyLog-RPA response
\begin{equation}
\chi^{(s)}(q,\omega)
=
\chi^L(q,\omega)\,
...
\label{eq:polylog-rpa}
\end{equation}
which recovers the standard RPA ladder content as $s\to 0$ and, at the catastrophic boundary $x\equiv v\chi^L\to 1$ with $\Re s>1$, converges to the Riemann zeta function:
$\lim_{x\to 1}\mathrm{Li}_s(x)=\zeta(s)$.
Pathway~A is the microscopic definition of the regularized vertex sum for a \emph{scalar} interaction ladder; it is not the production evaluator used for the numerical campaign of Sec.~\ref{sec:results}.
In particular, PolyLog-RPA does not extend naively to the coupled two-component RPA tensor:
there the geometric resummation is matrix-valued,
$\boldsymbol{\chi}^{\mathrm{RPA}}=\boldsymbol{\chi}^{L}(\mathbf{1}-\mathbf{v}\boldsymbol{\chi}^{L})^{-1}$,
so a scalar factor $\mathrm{Li}_s(v\chi^L)$ cannot be applied channel-by-channel without breaking interspecies entanglement, dielectric determinant structure, and the audited weak-coupling matrix limit.
A matrix polylogarithm $\mathrm{Li}_s(\mathbf{v}\boldsymbol{\chi}^{L})$ would require a separate spectral calculus (eigenbranch choices, non-Hermitian continuation) outside the scope of the present production campaign.

\paragraph{Pathway B --- Zeta-RPA / LFC (production Ansatz).}
For macroscopic plasma diagnostics we retain the standard local-field-correction (LFC) skeleton,
dressing the bare interaction with a global weight $W_\zeta\equiv 1-G_\zeta$.
For a single species with bare interaction $v(q)$, the production response and dielectric function read
\begin{equation}
\chi^{\zeta}(q,\omega)
=
...
=
1 - v(q)\,W_\zeta\,\chi^L(q,\omega)\,,
\label{eq:zeta-rpa}
\end{equation}
with the locked production weight
\begin{equation}
W_\zeta(\Gamma,r_s,\tau)
=
\frac{\zeta\!\bigl(1+f\bigr)}{\zeta(1)}\,,
\qquad
f
=
\alpha\frac{\Gamma^{\beta}}{1 + \gamma\, r_s^{-\delta}\,\tau}\,.
\label{eq:zeta-weight}
\end{equation}
The coefficients $(\alpha,\beta,\gamma,\delta)$ are recorded in Appendix~\ref{sec:zeta-weight-coefficients}.
Because $\zeta(1)$ is divergent, Eq.~(\ref{eq:zeta-weight}) is evaluated in production through its
Laurent-regularized equivalent: $W_\zeta=1$ at $f=0$ ($\Gamma\to 0$), and
$W_\zeta=f\,\zeta(1+f)$ for $f>0$, with $\zeta(s)$ obtained from Borwein's convergent series~\cite{Edwards2001}
at a pinned truncation depth.
This construction enforces the exact weak-coupling identity
$W_\zeta\to 1\Rightarrow\chi^{\zeta}\equiv\chi^{\mathrm{RPA}}$ (scalar)
and, critically, never renegotiates causality:
$\Im\chi^L$ remains the grand-canonical dissipative kernel of Sec.~\ref{sec:finite-temperature-Lindhard},
and $\Re\chi^L$ continues to be obtained solely as its Hilbert transform.

\paragraph{The bridge.}
Pathways~A and~B are not algebraically identical on the full $(q,\omega)$ plane.
They are joined by the asymptotic projection of Appendix~\ref{sec:zeta-weight-coefficients}:
in the strong-coupling catastrophe $x\to 1$, the PolyLog boundary $\zeta(s)$ fixes the singular mass that the LFC geometric series $\sum(Wx)^n$ must reproduce, thereby motivating a zeta-determined macroscopic weight $W_\zeta$ while leaving the production evaluator inside the familiar $1/(1-Wx)$ plasma skeleton.
That LFC skeleton, unlike the scalar polylogarithm, promotes immediately to channel-resolved weights $W_\zeta^{st}$ on the two-component tensor (Sec.~\ref{sec:zeta-rpa-matrix}) without inventing a matrix $\mathrm{Li}_s$.
Unless otherwise stated, every numerical Zeta-RPA figure and table in this work is generated on Pathway~B; Pathway~A supplies the formal scalar diagrammatic definition and the topological boundary condition for that projection.

Plasmon poles of the scalar dielectric are extracted by the same derivative-free Brent contract~\cite{Brent1973}
used in Sec.~\ref{sec:plasmon-pole-extraction}, now applied to $\Re\varepsilon^{\zeta}(q,\omega)=0$,
with Landau damping reported as $\Im\varepsilon^{\zeta}$ on the extracted root.
Figure~\ref{fig:zeta-rpa-dispersion} contrasts the classical Bohm--Gross long-wavelength anchor~\cite{Bohm-Gross1949},
the scalar RPA dispersion $\omega_p^{\mathrm{RPA}}(q)$, the Pathway~A (PolyLog-RPA) branch $\omega_p^{(s)}(q)$,
and the Pathway~B (Zeta-RPA) branch $\omega_p^{\zeta}(q)$
at strong coupling ($\Gamma=100$, $r_s=2.0$, $T=10\,000\,\mathrm{K}$).
Where the scalar RPA root bracket collapses or the branch terminates prematurely,
the zeta-dressed dielectric retains a continuous $\Re\varepsilon^{\zeta}=0$ trajectory,
with a correspondingly modified on-pole dissipation spectrum
$|\Im\varepsilon^{\zeta}|$ [Fig.~\ref{fig:zeta-rpa-dispersion}(b)],
while the pure PolyLog series exhibits the continuum over-tracking discussed below.
The scalar construction above is the controlled single-component diagnostic;
the production two-component pathway is the asymmetric zeta matrix of
Sec.~\ref{sec:zeta-rpa-matrix}.
The physical necessity of this dual-pathway architecture is starkly revealed during scalar plasmon pole extraction at extreme coupling (e.g., $\Gamma=100$).
While Pathway~A (PolyLog-RPA) furnishes the rigorous microscopic diagrammatic summation, forcing this exact series deep into the strongly coupled regime causes the roots to unphysically track spurious resonances and extreme Landau damping tails deep into the single-particle continuum.
Pathway~B (LFC-Zeta), by contrast, projects the exact topological boundary of the Riemann zeta function onto a macroscopic local-field skeleton.
This projection intrinsically suppresses the spurious microscopic tracking artifacts of Pathway~A, yielding a highly stable, continuous collective mode and a properly regularized dissipation spectrum.
This pronounced diagnostic contrast strictly justifies the designation of Pathway~B as the production evaluator for all multi-component structure factors in this campaign.

\subsection{Multi-component Promotion: The Asymmetric Zeta Matrix}
\label{sec:zeta-rpa-matrix}

Extreme mass asymmetry ($m_i\gg m_e$) implies that electrons and ions occupy
independent thermodynamic regimes $(\Gamma_e,r_{s,e},\tau_e)$ and
$(\Gamma_i,r_{s,i},\tau_i)$ even at a common physical temperature.
A faithful strong-coupling promotion of Eqs.~(\ref{rpa-responses}) must therefore
dress each Coulomb channel with its own zeta weight while preserving the audited
algebraic skeleton of the undressed RPA tensor.
We define the dressed potentials
\begin{equation}
v_{st}^{\zeta}(q)
=
v_{st}(q)\,W_\zeta^{st}\,,
\qquad
st\in\{ee,ii,ei\}\,,
\label{eq:zeta-dressed-potentials}
\end{equation}
where the diagonal weights $W_\zeta^{ee}$ and $W_\zeta^{ii}$ are evaluated from
Eq.~(\ref{eq:zeta-weight}) on the electron and ion regimes, respectively.
The off-diagonal cross channel cannot inherit either species regime alone without
breaking $v_{ei}=v_{ie}$; we therefore assign the arithmetic-mean regime
\begin{equation}
\Gamma_{ei}
=
\frac{\Gamma_e+\Gamma_i}{2}\,,
\qquad
r_{s,ei}
=
\frac{r_{s,e}+r_{s,i}}{2}\,,
\qquad
\tau_{ei}
=
\frac{\tau_e+\tau_i}{2}\,,
\label{eq:zeta-cross-regime}
\end{equation}
and set $W_\zeta^{ei}=W_\zeta(\Gamma_{ei},r_{s,ei},\tau_{ei})$.
This choice preserves algebraic symmetry of the interaction matrix and enforces
weak-coupling continuity on every channel,
\begin{equation}
\lim_{\Gamma\to 0} W_\zeta^{st}=1
\qquad\Rightarrow\qquad
v_{st}^{\zeta}\to v_{st}\,.
\label{eq:zeta-matrix-weak-coupling}
\end{equation}

Substituting $v_{st}\to v_{st}^{\zeta}$ into the two-component dielectric determinant
yields
\begin{equation}
\varepsilon^{\zeta}(q,\omega)
=
...
\label{eq:zeta-dielectric-matrix}
\end{equation}
and the electron--electron tensor element
\begin{equation}
...
\chi_e^L(q,\omega)
-
\chi_e^L(q,\omega)\,v_{ii}^{\zeta}(q)\,\chi_i^L(q,\omega)
}{\varepsilon^{\zeta}(q,\omega)}\,,
\label{eq:zeta-chi-ee}
\end{equation}
with $\chi_{ii}^{\zeta}$ and $\chi_{ei}^{\zeta}$ obtained by the identical substitution
into Eqs.~(\ref{response-ii})--(\ref{response-ei}).
Equations~(\ref{eq:zeta-dressed-potentials})--(\ref{eq:zeta-chi-ee}) make the
architectural doctrine explicit:
the bare Lindhard susceptibilities $\chi_e^L$ and $\chi_i^L$ remain the immutable
Kramers--Kr\"onig / sinc inputs of Sec.~\ref{sec:finite-temperature-Lindhard};
only the interaction ladder is dressed.
In the production MosaiQ pathway, static and dynamic structure factors are assembled
from this asymmetric zeta matrix, recovering the undressed RPA tensor
Eqs.~(\ref{rpa-responses}) identically whenever all $W_\zeta^{st}\to 1$.
...
Although Dandrea \textit{et al.}~\cite{Dandrea1986} proposed a heuristic approach to numerical integration,
...
physics~\cite{Ichimaru1973,Giuliani-Vignale2005,Kremp2010,Fetter-Walecka1971,Pines1962}, including Pines and Nozi\`eres~\cite{Pines-Nozieres1989}.
...
Figs.~\ref{fig:thermal-anatomy-a} and~\ref{fig:thermal-anatomy-b} contrast $T=1\,000\,\mathrm{K}$ with an extreme cryogenic $T=0.01\,\mathrm{K}$ run; this cold rung was selected to stress the reverse-Dedekind singularity excision of Sec.~\ref{sec:singularity-excision} on a complete $(\bar{q},\bar{\omega})$ grid, confirming that the bare manifold remains fully populated without \texttt{NaN}/\texttt{Inf} collapse or artificial sparsity.
Fig.~\ref{fig:thermal-anatomy-c} overlays $T=0.01\,\mathrm{K}$, $1\,000\,\mathrm{K}$, and $10\,000\,\mathrm{K}$ at $\bar{q}=1.0$: at this coupling the three finite-$T$ curves are thermodynamically coincident and are rendered with distinct linestyles and staggered markers ($\circ$, $\square$, $\triangle$) so their overlap remains legible, isolating the physical residual sharpening visible only against the strict $T=0$ analytic real susceptibility (solid black).
...
At $T=0.01\,\mathrm{K}$ the engine sits deep inside the excised Stratonovich window ($\tau_e \sim 2\times 10^{-8}$), yet the $\bar{q}=1.0$ slice remains finite and tracks the causality-pinned $T=0$ analytic real part.
...
Standard implementations of contour integration over Matsubara frequencies that evaluate $\Re \chi^L$ through divergent pole summations~\cite{Khanna-Glyde1976,Gouedard-Deutsch1978},
...
When the numerical extraction is compared along that slice at $r_s = 2.0$, $\bar{q} = 1.0$, and $T \approx 11\,\mathrm{K}$ (Fig.~\ref{fig:t0-1d-slice}), the MosaiQ-Lindhard engine output (open markers) tracks a well-defined, repeatable trajectory, while the contour-based textbook reference of Refs.~\cite{Giuliani-Vignale2005,Gouedard-Deutsch1978} displays $\Re \tilde{\chi}_{T=0}$ as solid curves and $\Im \tilde{\chi}_{T=0}$ as dashed curves on a distinct manifold.
As shown in the error panel of Fig.~\ref{fig:t0-1d-slice}, $|\Delta \Re|$ (solid) remains at the $\mathcal{O}(1)$ level, whereas $|\Delta \Im|$ (dashed) is confined to the truncation floor of the sinc evaluation.

\subsection{Topological discrepancy and the Stratonovich imperative}
\label{sec:stratonovich-imperative}

This $\mathcal{O}(1)$ offset must not be interpreted as a numerical artifact or a defect of the MosaiQ-Lindhard engine.
Rather, it is the profound manifestation of a topological disconnect---which we term the \emph{reverse Dedekind cut}~\cite{Dedekind1872,Rudin1976_Cut}---superimposed on the ensemble inconsistency diagnosed in Sec.~\ref{sec:finite-temperature-Lindhard}.
As the temperature approaches absolute zero ($T \to 0$), the discrete Matsubara frequencies $\omega_n = (2n+1)\pi k_\mathrm{B} T$ collapse toward the real axis.
However, due to the intrinsic odd-integer factor $(2n+1)$ characteristic of fermions, these frequencies rigorously bracket, yet perpetually evade, the exact zero-energy origin ($\omega = 0$).

This reverse Dedekind cut implies that the $T=0$ limit of the Fermi--Dirac distribution is not merely a naive Heaviside function.
In his seminal treatise on path integrals~\cite{Kleinert2009}, Hagen Kleinert rigorously identifies three distinct regularizations for the step function at the origin.
To preserve Kramers--Kr\"onig causality across this topological gap, the macroscopic $T \to 0$ distribution is strictly forced into the \emph{Stratonovich} form, taking the exact midpoint value $f(0) = 1/2$.

Consequently, driving the numerical integration toward $T=0$ along the real axis forces the IEEE~754 double-precision quadrature to cross this Stratonovich anchor.
The extreme gradients in this vicinity inevitably trigger catastrophic cancellation.
Resolving this requires an analytical excision of the singularity, detailed in Sec.~\ref{sec:singularity-excision}, which enables a smooth numerical continuation deep into the cryogenic regime.
Figure~\ref{fig:excision-heatmap} documents the outcome on the metallic $(r_s, T)$ plane: with singularity excision engaged, every cell remains finite and the relative deviation from the causality-pinned $T=0$ baseline is bounded by $\sim 10^{-11}$.
The engine therefore remains stable and convergent in the $T \rightarrow 0$ limit \emph{within} the causality-constrained space;
the residual $\mathcal{O}(1)$ offset against the contour-based textbook real part of Refs.~\cite{Giuliani-Vignale2005,Gouedard-Deutsch1978} is the expected price of refusing to contaminate that space with thermodynamically inconsistent analytic shortcuts (Sec.~\ref{sec:t0-kk-contrast}).
...
with each $\Gamma$ encoded by color and by a linestyle progression from dot-dot-dashed ($\Gamma = 10$) and dot-dashed ($\Gamma = 50$) through dashed ($\Gamma = 100$) to solid ($\Gamma = 150$).
...
The high-resolution insets in Fig.~\ref{fig:Sq-gamma} provide an anatomical dissection of the structural response in the microscopic sector under extreme coupling ($\Gamma \ge 100$).
In Fig.~\ref{fig:Sq-gamma-ee}, the inset spanning $\bar{q} \in [1.15, 2.20]$ magnifies the Wigner crystallization precursor sector near $\bar{q} \approx 1.8$ and reveals the progressive heightening of the first coordination shell as $\Gamma$ increases.
Fig.~\ref{fig:Sq-gamma-ii} resolves the heavy-ion coordination spikes with the inset extended to $\bar{q} \le 8.0$, while Fig.~\ref{fig:Sq-gamma-ei} captures the phase-shifted, suppressed electron-drag peak.
The induction of that $S_{ei}$ feature, despite its small magnitude ($\mathcal{O}(10^{-3})$), quantifies the exact electron-drag effect where the light electron fluid adheres to the heavy-ion acoustic manifold.
...
\section{Numerical Architecture for Electron-Phonon Supergraphs}
\label{sec:berezinian_numerics}

\subsection{Topological overcounting and graded supermatrices}
In the strong-coupling regime of semiconductor physics (e.g., silicon), the standard random-phase approximation (RPA)~\cite{RPA} catastrophically breaks down due to the topological overcounting of composite ring diagrams~\cite{Fetter-Walecka1971,Holstein1959}.
This manifests macroscopically as the uniform hardening of phonon spectra and the breakdown of the Born--Oppenheimer approximation~\cite{Ashcroft-Mermin1976}.

To resolve this analytically and numerically, we treat the coupled electron--phonon system as a \textit{graded supergraph}.
The statistical sign difference between fermions (electrons) and bosons (phonons) requires substituting the conventional matrix determinant with the Berezinian (superdeterminant)~\cite{Berezin1987,DeWitt1992}.
For an asymmetric supermatrix $\mathcal{M}$ coupling the electronic block $A_{ee}$ and the phononic block $D_{pp}$ via off-diagonal interactions $B_{ep}$ and $C_{pe}$,
\begin{equation}
\mathcal{M}
=
\begin{pmatrix}
...
\begin{equation}
\label{eq:berezinian-schur}
\operatorname{sdet}(\mathcal{M})
=
\frac{\det\!\bigl(A_{ee} - B_{ep} D_{pp}^{-1} C_{pe}\bigr)}{\det(D_{pp})}.
\end{equation}

\subsection{The $DX = C$ strategy and zero-dependency factorization}
A naive computational approach attempts to form $D_{pp}^{-1}$ explicitly.
However, explicit inversion is numerically unstable and computationally wasteful~\cite{NR,Golub-VanLoan2013}.
The MosaiQ engine therefore enforces a strict $DX = C$ strategy:
we solve the linear system $D_{pp} X = C_{pe}$, and construct the Schur complement by a single matrix multiplication
$\tilde{A} = A_{ee} - B_{ep} X$.

Since the phonon dynamical matrix $D_{pp}$ is inherently symmetric, we utilize an in-place LDLT factorization~\cite{NR,Golub-VanLoan2013}.
The required scalar determinant $\det(D_{pp})$ is harvested directly from the diagonal elements of the factorized matrix ($\prod_i d_{ii}$) at zero additional $\mathcal{O}(N^3)$ cost, after which
\begin{equation}
\operatorname{sdet}(\mathcal{M})
=
\frac{\det(\tilde{A})}{\det(D_{pp})}
\end{equation}
is evaluated from the same factors without a second cubic factorization of $D_{pp}$.

\subsection{Retarded regularization for Keldysh causality}
The strict preservation of physical causality on the real-frequency axis (the Keldysh contour)~\cite{Kadanoff1962,Stefanucci2013,Kim2018} demands that the poles of the response function remain analytically regulated.
Before factorization, a retarded regularization term $i\eta$ is loaded onto the diagonal of the phonon block:
\begin{equation}
\label{eq:retarded-eta}
D_{\mathrm{reg}}
=
D_{pp} + i\eta\,\mathbf{I}.
\end{equation}
Unit tests within the engine confirm that this diagonal loading stabilizes the Berezinian against singular configurations (e.g., $\det(D_{pp})\to 0$) without triggering NaN propagation, thereby preserving the structural integrity of the causal response.
The corresponding C++20 interfaces---\texttt{SuperMatrix.hpp}, \texttt{LinalgUtils.hpp}, and \texttt{Berezinian.hpp}---are archived with the MosaiQ-Lindhard release cited in the Data and Code Availability statement.
...
this constitutes a resolution, within the RPA Lindhard class~\cite{Lindhard1954},
of the analytic inconsistencies in contour integration that are long-standing in the literature
...
without the distortions introduced by heuristic simplifications, order interchanges not justified by Fubini's theorem,
or ensemble-inconsistent response formulations.
...
recovering the exact $T=0$ Lindhard benchmark~\cite{Lindhard1954} as a special case of the grand-canonical Hilbert-transform route.
The Zeta-RPA extension of Sec.~\ref{sec:zeta-rpa} carries that healed bare response into the strongly coupled liquid by dressing $v(q)$ with the locked weight $W_\zeta$ of Eq.~(\ref{eq:zeta-weight}), without resurrecting Matsubara pole sums or altering $\Im\chi^L$; Fig.~\ref{fig:zeta-rpa-dispersion} documents the three-way scalar plasmon cross-check (Standard RPA / PolyLog-RPA / Zeta-RPA) at $\Gamma=100$, while Sec.~\ref{sec:zeta-rpa-matrix} promotes the Zeta dress to the production two-component asymmetric zeta matrix.
Sec.~\ref{sec:berezinian_numerics} further freezes the zero-dependency Schur/$DX{=}C$/$i\eta$ architecture that elevates the same causal engine from two-fermion RPA tensors to electron--phonon graded supergraphs via the Berezinian of Eq.~(\ref{eq:berezinian-schur}).
...
The deterministic C++20 engine (MosaiQ-Lindhard) and the associated Python visualization scripts used to generate the results in this study are open-source and publicly available. To facilitate independent verification of the macroscopic sum rules, the deterministic plasmon dispersion extraction, and the mathematical breakdown of the standard contour-integration routes of Refs.~\cite{Giuliani-Vignale2005,Gouedard-Deutsch1978} at the highly degenerate $T \to 0$ limit, readers are strongly encouraged to clone the repository and reproduce the findings. The complete source code, test suites, and build instructions are available on GitHub at \url{https://github.com/BonaSapiensInc/MosaiQ-Lindhard} and permanently archived on Zenodo at \url{https://doi.org/10.5281/zenodo.21438736} (release \texttt{v2.0.0-dual-pathway}).
...
\section{Dual pathways: PolyLog-RPA definition and Zeta-RPA production weights}
\label{sec:zeta-weight-coefficients}

This appendix records the locked coefficients of the production weight $W_\zeta$ and, more importantly, the dual-pathway architecture of Sec.~\ref{sec:zeta-rpa}: Pathway~A supplies the formal diagrammatic definition, Pathway~B the macroscopic LFC evaluator used throughout the numerical campaign, and the $x\to 1$ asymptotic matching joins the two without claiming algebraic identity on the full $(q,\omega)$ mesh.

\paragraph{Pathway A --- PolyLog-RPA.}
Ordinary RPA sums single-bubble polarization diagrams to infinite order. With $x\equiv v\chi^L$, the interaction ladder content of that sum is the polylogarithm cross-section $\mathrm{Li}_0(x)$. Promoting the same ladder by a continuous topological penalty $n^{-s}$ defines the PolyLog-RPA response
\begin{equation}
\chi^{(s)}(q,\omega)
=
\chi^L(q,\omega)\,
...
\label{eq:appendix-polylog-response}
\end{equation}
which is the exact series definition adopted in this work for the regularized \emph{scalar} vertex sum.
As $s\to 0$ one recovers the RPA ladder; for $\Re s>1$ and $x\to 1$ the series converges to the Riemann zeta boundary,
\begin{equation}
\lim_{x\to 1}\mathrm{Li}_s(x)=\zeta(s)\,.
\label{eq:appendix-polylog-zeta-boundary}
\end{equation}
The same construction does not furnish a production rule for coupled multicomponent RPA.
There the bubble resummation is organized by the matrix geometric series
$(\mathbf{1}-\mathbf{v}\boldsymbol{\chi}^{L})^{-1}$,
so neither a channel-wise scalar $\mathrm{Li}_s(v_{st}\chi_{st}^{L})$ nor an unexamined matrix function $\mathrm{Li}_s(\mathbf{v}\boldsymbol{\chi}^{L})$ preserves, without additional spectral hypotheses, the interspecies structure of Eqs.~(\ref{rpa-responses}) and the dielectric determinant.
Pathway~A is therefore retained as the scalar formal definition and asymptotic boundary supplier; the two-component campaign uses Pathway~B.

\paragraph{Pathway B --- LFC--Zeta production.}
Evaluating Eq.~(\ref{eq:appendix-polylog-response}) over the full causal mesh is unnecessary for the production diagnostics of Sec.~\ref{sec:results}, departs from the LFC algebraic language standard in plasma physics, and---as just noted---does not extend cleanly to the coupled electron--ion tensor.
Pathway~B therefore retains the macroscopic skeleton
\begin{equation}
\chi^{\zeta}
=
\frac{\chi^L}{1-v\,W_\zeta\,\chi^L}\,,
\qquad
W_\zeta\equiv 1-G_\zeta\,,
\label{eq:appendix-lfc-skeleton}
\end{equation}
i.e.\ RPA with a static local-field weight, and commits all reported multi-component figures to this evaluator.

\paragraph{Asymptotic bridge.}
The two pathways meet at the catastrophe boundary. The LFC geometric series $\sum_{n=1}^{\infty}(W x)^n$ accumulates the singular mass $W/(1-W)$ as $x\to 1$, while Pathway~A accumulates $\zeta(s)$. Equating these macroscopic singular masses,
\begin{equation}
\lim_{x \to 1} \mathrm{Li}_s(x) = \zeta(s) \quad \longleftrightarrow \quad \lim_{x \to 1} \sum_{n=1}^{\infty} (W x)^n = \frac{W}{1-W} \,,
\label{eq:appendix-asymptotic-mapping}
\end{equation}
motivates determining the production weight from the absolutely convergent zeta sector rather than from an empirical bridge function.
Zeta-RPA implements that motivation with the regularizing argument $s=1+f>1$, where
\begin{equation}
f = \alpha \frac{\Gamma^\beta}{1 + \gamma r_s^{-\delta} \tau} \,,
\label{eq:appendix-f-definition}
\end{equation}
quantifies the thermodynamic distance from the weak-coupling pole: $\Gamma^\beta$ amplifies vertex corrections under strong coupling, while the denominator quenches them at large $\tau$ or large $r_s$, driving $f\to 0$ and returning Pathway~B to undressed RPA.

Unless otherwise stated, all numerical multi-component Zeta-RPA results in this study utilize the Pathway~B production baseline committed in the live C++ engine:
\begin{equation}
\alpha = 0.05 \,, \quad \beta = 1 \,, \quad \gamma = 1 \,, \quad \delta = 1 \,,
\label{eq:zeta-weight-coefficients}
\end{equation}
evaluated through the Laurent-regularized representation $W_\zeta = f \zeta(1+f)$ for $f > 0$, guaranteeing a seamless, parameter-free collapse to the audited two-component legacy RPA tensor whenever $\Gamma \to 0$.


\section{Exact $T=0$ Lindhard Function via Kramers--Kr\"onig Reconstruction}
\label{sec:Lindhard-function}

One of the central claims of this work is that the finite-temperature formulation maintains exact causality. To demonstrate this analytic continuity explicitly, Appendix~\ref{sec:Lindhard-function} shows that the well-known zero-temperature Lindhard function can be recovered purely from the imaginary part through the Kramers--Kr\"onig relation, without invoking contour integration in the complex plane. Sec.~\ref{sec:t0-kk-reconstruction} derives the exact $T=0$ imaginary and real parts on the real axis, Sec.~\ref{sec:t0-kk-contrast} contrasts this reconstruction with standard contour and pole-summation routes, including the $\mathcal{O}(1)$ real-part offset of Fig.~\ref{fig:t0-1d-slice}, and Sec.~\ref{sec:singularity-excision} supplies the reverse-Dedekind / Stratonovich singularity excision that stabilizes the cryogenic quadrature.
...
Direct substitution of the Heaviside step function $\Theta(1-\kappa)$ into the complex contour integral reintroduces the simple-pole problem on the real axis and violates Jordan's lemma.
Instead, we proceed strictly via the imaginary part, utilizing its compact support to restrict the principal-value integral to finite intervals, thereby recovering the standard logarithmic form analytically without complex excursions.
Evaluating the causality-constrained imaginary part Eq.~(\ref{imaginary-Lindhard}) strictly on the real axis yields the exact compact-support $T=0$ limit~\cite{Lindhard1954}
\begin{equation}
...
\subsection{Analytical excision of the Stratonovich singularity}
\label{sec:singularity-excision}

To circumvent the catastrophic \texttt{NaN}/\texttt{Inf} divergence near the Stratonovich anchor ($f(0)=1/2$), the MosaiQ-Lindhard engine dynamically switches to a singularity-excision algorithm when the reduced temperature falls below $\tau < 0.02$.
Above this threshold the legacy Kramers--Kr\"onig sinc path is retained bit-for-bit, preserving high-temperature reproducibility.

The essence of the method is the softplus decomposition of the scaled Fermi window,
\begin{equation}
\tau \ln\bigl(1 + e^{x/\tau}\bigr)
= (x)_+
+ \tau \ln\bigl(1 + e^{-|x|/\tau}\bigr) ,
\label{softplus-stratonovich-decomposition}
\end{equation}
where $x = \gamma - s^2$ is the kinematic deviation from the Fermi surface in the Hilbert variable of Eq.~(\ref{real-Lindhard-Kramers-Kronig}).
At $x=0$ the residual gracefully yields the Stratonovich midpoint $\tau \ln 2$, enforcing $f(0)=1/2$ as required by the reverse Dedekind cut of Sec.~\ref{sec:stratonovich-imperative}.
By evaluating the Hilbert transform of the singular piecewise component $(x)_+$ in closed form, we obtain an analytic contribution to $\Re\chi^L$.
The numerical sinc quadrature is then fed exclusively the bounded, smooth residual
\begin{equation}
R_\tau(x) = \tau \ln\bigl(1 + e^{-|x|/\tau}\bigr) .
\label{softplus-smooth-residual}
\end{equation}
This excision fully immunizes the Kramers--Kr\"onig integral against the diverging gradients of the reverse Dedekind cut and eliminates floating-point underflow/overflow in the cryogenic metallic window documented in Fig.~\ref{fig:excision-heatmap}.
...
\bibitem{Lindhard1954} J. Lindhard, On the Properties of a Gas of Charged Particles,
\textit{K. Dan. Vidensk. Selsk. Mat.-Fys. Medd.} \textbf{28}, No.~8 (1954).
...
\bibitem{Kohn1959} W. Kohn, Phys. Rev. Lett. \textbf{2}, 393 (1959).
...
\bibitem{Rudin1976} W. Rudin, \textit{Principles of Mathematical Analysis}, 3rd ed.
(McGraw-Hill, New York, 1976), Theorem~11.32.
...
\bibitem{Kleinert2009} H. Kleinert, \textit{Path Integrals in Quantum Mechanics, Statistics, Polymer Physics, and Financial Markets, 5th Edition}
...
\bibitem{Dedekind1872} R. Dedekind, \textit{Stetigkeit und irrationale Zahlen}
(Vieweg, Braunschweig, 1872)
[English translation: W. W. Beman, \textit{Essays on the Theory of Numbers}
(Open Court, Chicago, 1901)].

\bibitem{Rudin1976_Cut} W. Rudin, \textit{Principles of Mathematical Analysis}, 3rd ed.
(McGraw-Hill, New York, 1976), Chap.~1.
...
\bibitem{Berezin1987} F. A. Berezin, \textit{Introduction to Superanalysis}, edited by A. A. Kirillov
(D. Reidel, Dordrecht, 1987).

\bibitem{DeWitt1992} B. DeWitt, \textit{Supermanifolds}, 2nd ed. (Cambridge University Press, Cambridge, 1992).

\bibitem{Golub-VanLoan2013} G. H. Golub and C. F. Van Loan, \textit{Matrix Computations}, 4th ed.
...
\begin{itemize}
    ...
    \item \textbf{FIG. 2.} Three-way scalar plasmon cross-check at strong coupling ($\Gamma = 100$, $r_s = 2.0$, $T = 10\,000\,\mathrm{K}$), constructed from the deterministic dielectric roots of Sec.~\ref{sec:zeta-rpa}.
    Fig.~\ref{fig:zeta-rpa-dispersion}(a) overlays the classical Bohm--Gross long-wavelength anchor~\cite{Bohm-Gross1949} (gray dashed), the standard scalar RPA dispersion $\bar{\omega}_p^{\mathrm{RPA}}(\bar{q})$ from $\Re\varepsilon^{\mathrm{RPA}}=0$ (crimson), the Pathway~A (PolyLog-RPA) branch $\bar{\omega}_p^{(s)}(\bar{q})$ from $\Re\varepsilon^{(s)}=0$ (teal), and the Pathway~B (Zeta-RPA) branch $\bar{\omega}_p^{\zeta}(\bar{q})$ from $\Re\varepsilon^{\zeta}=0$ (blue) on the reduced wavevector window $\bar{q}\le 4$.
    The shaded continuum region marks wave vectors where the standard RPA root bracket collapses; Pathway~A continues to track spurious continuum resonances, whereas the zeta-dressed dielectric retains a continuous, physically stable collective trajectory.
    Fig.~\ref{fig:zeta-rpa-dispersion}(b) reports the corresponding on-pole Landau damping $|\Im\varepsilon|$, demonstrating that the macroscopic LFC projection of Eq.~(\ref{eq:zeta-weight}) suppresses the extreme PolyLog dissipation artifacts while healing the many-body divergence without altering the underlying Kramers--Kr\"onig-constrained Lindhard input $\chi^L$.

    \item \textbf{FIG. 3.} Schematic positions of the infinite series of simple poles
    ...
    \item \textbf{FIG. 4.} The non-interacting base manifolds of the finite-temperature Lindhard response functions $\chi_{st}^L(\bar{q}, \bar{\omega})$ at $r_s = 1.0$ and $T = 10\,000\,\mathrm{K}$, resampled onto channel-specific display rectangles with integer axis limits; ion panels sample the causal export on $\bar{q} \le 50.0$ but display the Golden Scale $\bar{q} \in [0, 20.0]$, $\bar{\omega} \in [0, 0.20]$.
    ...
    \item \textbf{FIG. 5.} Multi-component Zeta-RPA dynamic structure factors from the MosaiQ-Lindhard deterministic sandbox at $r_s = 1.0$ and $T = 10\,000\,\mathrm{K}$, incorporating the species-asymmetric interaction dressing of Sec.~\ref{sec:zeta-rpa-matrix} and using the same display-frame convention as Fig.~\ref{fig:bare-lindhard}.
    ...
    Fig.~\ref{fig:S_ii} shows $|S_{ii}|$ on the ion Golden Scale $\bar{q} \in [0, 20.0]$ and $\bar{\omega} \in [0, 0.20]$ (resampled from the full causal mesh to $\bar{q} = 50.0$).
    Unlike standard weakly coupled RPA, the Zeta-RPA formulation regularizes the strong-coupling divergence channel-by-channel through the asymmetric weights $W_\zeta^{st}$.
    This reveals a stabilized electron plasmon branch in $S_{ee}$, noise-free resonant pockets in the ion-acoustic manifold of $S_{ii}$, and a pristine low-frequency electron-drag entanglement in $S_{ei}$, free from the numerical scattering artifacts that conventionally plague strongly coupled two-component models.
    Each panel maps $|S_{st}|$ on a base-10 logarithmic scale from $S_{\min}=\max(S_{>0},10^{-12})$ to $S_{\max}$ with $N_{\mathrm{fill}}=200$ log-uniform levels and white isolines $N_{\mathrm{line}}=72$; Fig.~\ref{fig:S_ei} uses the bold cross-channel isoline profile of Fig.~\ref{fig:chi_ei_im}.

    \item \textbf{FIG. 6.} Extracted electron plasmon dispersion trajectory and corresponding Landau damping for a partially degenerate plasma at $r_s = 2.0$ and $T = 10\,000\,\mathrm{K}$.
    ...
    \item \textbf{FIG. 7.} Highly degenerate limit validation at extreme degeneracy ($r_s = 2.0$, $\tau = 10^{-4}$, $T \approx 11\,\mathrm{K}$). Figs.~\ref{fig:t0-re-2d} and~\ref{fig:t0-im-2d} show the causality-pinned $T = 0$ Lindhard susceptibility topology over the $(\bar{q}, \bar{\omega})$ plane, derived via the Kramers--Kr\"onig reconstruction of Appendix~\ref{sec:Lindhard-function}, rendered as base-10 logarithmic contour maps of $|\Re \tilde{\chi}^L_{T=0}|$ and $|\Im \tilde{\chi}^L_{T=0}|$ with the same white isoline convention as Fig.~\ref{fig:bare-lindhard}. A black dashed vertical guide marks the $\bar{q} = 1.0$ slice used in Fig.~\ref{fig:t0-1d-slice}. In Fig.~\ref{fig:t0-1d-slice}, the top panel extracts the 1D slice at $r_s = 2.0$, $\bar{q} = 1.0$, and $T \approx 11\,\mathrm{K}$, comparing the standard contour-based reference of Refs.~\cite{Giuliani-Vignale2005,Gouedard-Deutsch1978} ($\Re \tilde{\chi}_{T=0}$ solid, $\Im \tilde{\chi}_{T=0}$ dashed) against the MosaiQ-Lindhard engine (open markers); the bottom panel shows $|\Delta \Re|$ (solid) and $|\Delta \Im|$ (dashed) on a logarithmic scale. While $\Im \chi^L$ flawlessly converges to the $\tau \rightarrow 0$ limit, $\Re \chi^L$ retains an $\mathcal{O}(1)$ offset, visually capturing the ensemble inconsistency of standard contour integration over Matsubara frequencies discussed in Sec.~\ref{sec:finite-temperature-Lindhard}.

    \item \textbf{FIG. 8.} The anatomy of thermal sharpening for the real part of the electron susceptibility $\Re\chi_e^L(\bar{q}, \bar{\omega})$ at $r_s = 1.0$.
    Figs.~\ref{fig:thermal-anatomy-a} and~\ref{fig:thermal-anatomy-b} display the thermal sharpening ladder of $|\Re\chi_e^L|$ at intermediate ($T=1\,000\,\mathrm{K}$) and extreme cryogenic ($T=0.01\,\mathrm{K}$) conditions on the electron-scale rectangle $\bar{q}\in[0,4]$ and $\bar{\omega}\in[0,3]$, using the same base-10 logarithmic rainbow convention as Fig.~\ref{fig:bare-lindhard}; the $T=0.01\,\mathrm{K}$ panel fills the display frame on a numerically stable grid enabled by the Stratonovich singularity excision of Sec.~\ref{sec:singularity-excision}.
    Fig.~\ref{fig:thermal-anatomy-c} extracts a high-resolution 1D cross-section at $\bar{q}=1.0$, overlaying $T=0.01\,\mathrm{K}$ (solid with $\circ$), $1\,000\,\mathrm{K}$ (dashed with $\square$), and $10\,000\,\mathrm{K}$ (dash-dotted with $\triangle$)---thermodynamically coincident at this $r_s$ and distinguished only by linestyle and staggered markers---against the strict $T=0$ analytic real susceptibility (solid black) to isolate the genuine low-temperature sharpening of the logarithmic singularities.
    ...
    \item \textbf{FIG. 9.} Relative error of the regularized finite-temperature Lindhard response $\chi^L(r_s, T)$ against the causality-pinned $T=0$ analytic baseline at fixed probe $(\bar{q},\bar{\omega})=(1.0,0.5)$ with $\gamma=1$.
    Sweeping the metallic density $r_s \in [1, 5]$ over $T \in [10^{-2}, 10^{6}]\,\mathrm{K}$ typically yields ubiquitous \texttt{NaN}/\texttt{Inf} failures in IEEE~754 double arithmetic near the Stratonovich anchor at cryogenic temperatures.
    By applying the reverse-Dedekind singularity excision of Sec.~\ref{sec:singularity-excision}, the MosaiQ-Lindhard engine achieves absolute numerical stability (zero \texttt{NaN}/\texttt{Inf} cells) and preserves a maximum relative error bounded by $\sim 10^{-11}$ (Fig.~\ref{fig:excision-heatmap}).

    \item \textbf{FIG. 10.} Static structure factors: Fig.~\ref{fig:Sq-gamma-ee} ($S_{ee}(\bar{q})$), Fig.~\ref{fig:Sq-gamma-ii} ($S_{ii}(\bar{q})$), and Fig.~\ref{fig:Sq-gamma-ei} ($S_{ei}(\bar{q})$) obtained via direct frequency integration of the multi-component Zeta-RPA dynamic response for a dense plasma at $r_s = 1.0$.
    The temperature is deterministically mapped to sweep the coupling parameter $\Gamma \in \{10, 50, 100, 150\}$, with color denoting the coupling strength and linestyles progressing from dot-dot-dashed ($\Gamma = 10$) and dot-dashed ($\Gamma = 50$) through dashed ($\Gamma = 100$) to solid ($\Gamma = 150$).
    The main panels showcase the monotonic asymptotic convergence to the ideal-gas limit ($S \to 1$ or $0$) up to the macroscopic horizon $\bar{q} = 50.0$.
    High-resolution insets provide anatomical magnification of the microscopic sector under extreme coupling ($\Gamma \ge 100$), where the Riemann-zeta dressing of Sec.~\ref{sec:zeta-rpa-matrix} stabilizes the phase topology.
    Fig.~\ref{fig:Sq-gamma-ee} zooms into $\bar{q} \in [1.15, 2.20]$, capturing the Wigner crystallization precursor sector near $\bar{q} \approx 1.8$ as regularized by the diagonal weight $W_\zeta^{ee}$.
    Fig.~\ref{fig:Sq-gamma-ii} resolves distinct structural oscillations of the heavy-ion fluid, with the inset extended to $\bar{q} \le 8.0$ so that the extreme-coupling coordination spikes remain in frame.
    Crucially, Fig.~\ref{fig:Sq-gamma-ei} demonstrates the phase-shifted, suppressed electron-drag peak governed by the arithmetic-mean cross regime $\Gamma_{ei}$ of Eq.~(\ref{eq:zeta-cross-regime}), which analytically regularizes the interspecies entanglement without violating the weak-coupling continuity $W_\zeta^{st}\to 1$.
\end{itemize}
...
\centering
\vfill
\includegraphics[width=\textwidth]{figures/zeta_rpa_dispersion.pdf}
\vfill
\registerstandalonefigurelabel{fig:zeta-rpa-dispersion}
\figurepaneltag{Fig.~\thesubmitfig}
\end{figure}
...
\panelpagefigure[fig:bare-lindhard]{figures/chi_e_re.pdf}{fig:chi_e_re}
\panelpagefigure{figures/chi_e_im.pdf}{fig:chi_e_im}
\panelpagefigure{figures/chi_i_re.pdf}{fig:chi_i_re}
\panelpagefigure{figures/chi_i_im.pdf}{fig:chi_i_im}
\panelpagefigure{figures/chi_ei_re.pdf}{fig:chi_ei_re}
\panelpagefigure{figures/chi_ei_im.pdf}{fig:chi_ei_im}

\panelpagefigure[fig:structure-factors]{figures/S_ee_contour.pdf}{fig:S_ee}
\panelpagefigure{figures/S_ii_contour.pdf}{fig:S_ii}
\panelpagefigure{figures/S_ei_contour.pdf}{fig:S_ei}

\clearpage
\begin{figure}[p]
\centering
\vfill
\includegraphics[width=\textwidth]{figures/plasmon_dispersion.pdf}
\vfill
\registerstandalonefigurelabel{fig:plasmon-dispersion}
\figurepaneltag{Fig.~\thesubmitfig}
\end{figure}

\panelpagefigure[fig:t0-limit-validation]{figures/t0_analytic_re_contour.pdf}{fig:t0-re-2d}
\panelpagefigure{figures/t0_analytic_im_contour.pdf}{fig:t0-im-2d}
\panelpagefigure{figures/t0_limit_validation.pdf}{fig:t0-1d-slice}

\panelpagefigure[fig:thermal-anatomy]{figures/thermal_anatomy_a.pdf}{fig:thermal-anatomy-a}
\panelpagefigure{figures/thermal_anatomy_b.pdf}{fig:thermal-anatomy-b}
\panelpagefigure{figures/thermal_anatomy_c.pdf}{fig:thermal-anatomy-c}
\panelpagefigure{figures/thermal_anatomy_d.pdf}{fig:thermal-anatomy-d}

\clearpage
\begin{figure}[p]
\centering
\vfill
\includegraphics[width=\textwidth]{figures/lindhard_rs_t_excision.pdf}
\vfill
\registerstandalonefigurelabel{fig:excision-heatmap}
\figurepaneltag{Fig.~\thesubmitfig}
\end{figure}

\panelpagefigure[fig:Sq-gamma]{figures/Sq_gamma_ee.pdf}{fig:Sq-gamma-ee}
\panelpagefigure{figures/Sq_gamma_ii.pdf}{fig:Sq-gamma-ii}
\panelpagefigure{figures/Sq_gamma_ei.pdf}{fig:Sq-gamma-ei}
