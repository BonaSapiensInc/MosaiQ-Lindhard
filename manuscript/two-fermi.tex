...
%\thanks{Discussion with Michael S. Murilllo}
\author{In-Gee Kim}
...
\begin{abstract}
We present a deterministic, causality-constrained formulation for the finite-temperature Lindhard linear response and two-component random-phase approximation (RPA) dielectric functions, resolving analytic inconsistencies that have persisted in contour-integration methods. We demonstrate that conventional approaches---which rely on converting the real part of the susceptibility into infinite residue sums over Matsubara-like poles---violate the prerequisite conditions of Jordan's Lemma and suffer from divergent series tails. Furthermore, hybrid methods that combine grand-canonical dissipation with canonical contour sums introduce an ensemble inconsistency, breaking exact causality and resulting in $\mathcal{O}(1)$ real-part errors that manifest prominently as a persistent offset from the exact analytical solutions in the highly degenerate $T \to 0$ limit. To restore rigorous thermodynamic and analytic consistency, we map the evaluation into a strictly constrained physical space: the imaginary part is derived in closed form within the grand-canonical ensemble, and the real part is exclusively obtained via the Kramers--Kr\"onig relation. By employing a modified sinc-quadrature rule with a theoretical truncation error bound of $\mathcal{O}(\sqrt{N} \exp(-\sqrt{\pi d \alpha N}))$ (where $N$ is the truncation limit, $d$ is the half-width of the analytic strip, and $\alpha$ is a convergence parameter), we achieve an evaluation accuracy exponentially suppressed well below machine precision ($\sim 10^{-259}$ for the chosen parameters $N=16384$, $d/\alpha=7$). We validate this deterministic framework by extracting the electron plasmon dispersion and exact Landau damping via derivative-free root finding, and by demonstrating the precise satisfaction of four macroscopic dynamic structure factor sum rules within machine precision without ad hoc regularization or parameter tuning.
\end{abstract}
...
was established by Selchow \textit{et al.}~\cite{Selchow2001}.
...
Following the traditional analytic continuation implicitly employed by Khanna and Glyde, one might formally treat the squared momentum as a generic complex variable, $\theta = \kappa^2$. By invoking Euler's identity, $e^{i\pi} + 1 = 0$, the roots of the Fermi--Dirac denominator effortlessly emerge at $\theta_j = \gamma \pm i\pi\tau_j$. Mapping this back to the momentum space yields the complex squared-momenta of Eq.~(\ref{series-poles}). (As we will demonstrate in Sec.~\ref{sec:finite-temperature-Lindhard} and Appendix~\ref{sec:Gouedard-Deutsch}, however, this very formal maneuver---forcing $\kappa^2$ into the complex plane to enforce a contour closure---is the precise topological origin of the analytic breakdown and the divergent Matsubara-series tails. Physically, forcing the real momentum into these complex discrete poles implicitly shatters the single continuous grand-canonical ensemble into an unphysical discrete mixture of non-Hermitian states, a topological breakdown we identify as \textbf{ensemble fragmentation}.)
...
Condition~(ii) is the content of Jordan's Lemma \cite{Jordan1894,BrownChurchill2009}:
...
by a sum of residues~\cite{BrownChurchill2009}.
...
the arc integral does not vanish, and Jordan's Lemma \cite{Jordan1894,BrownChurchill2009} cannot be invoked.
Applying Cauchy's residue theorem while neglecting this non-vanishing arc contribution
...
it reflects a violation of the contour prerequisite inherent to the infinite Matsubara tower Eq.~(\ref{Matsubara-frequencies}).
...
by applying Euler-Maclaurin formula to the summation after a certain amount of direct summation,
...
A separate class of approximations in the literature~\cite{Giuliani-Vignale2005,Khanna-Glyde1976,Gouedard-Deutsch1978}
...
Such hybrid routes constitute an ensemble inconsistency:
...
The contour-based summation route shares a common failure mode with these ensemble-mixed approximations:
...
Neither route provides a convergent representation of $\Re\chi^L$.
...
This formulation bypasses the infinite Matsubara pole barrier entirely by redefining the evaluation domain into a strictly causal real-axis physical space, rather than attempting to sum over the divergent imaginary-frequency singularities:
...
but they are based on, for example, the dynamic structure factor sum rules~\cite{Pines-Nozieres1989},
\begin{subequations}
...
It is better to examine a more symmetric form of the structure factor~\cite{Pines-Nozieres1989}
\begin{equation}
...
physics~\cite{Pines-Nozieres1989,Giuliani-Vignale2005,Kremp2010,Fetter-Walecka1971,Pines1962,Ichimaru1973}.
...
evaluates the finite-temperature Lindhard functions via the Kramers-Kr\"onig / sinc-quadrature route of Sec.~\ref{sec:finite-temperature-Lindhard},
...
on a dense $(\bar{q}, \bar{\omega})$ grid ($\bar{q} \in [0.1, 4.0]$, $\bar{\omega} \in [0.01, 3.0]$ in electron Fermi units).

Figs.~\ref{fig:S_ee}--\ref{fig:S_ei_3d} summarize the three diagonal and cross-channel structure factors
$S_{ee}$, $S_{ii}$, and $S_{ei}$ obtained from this run, with each channel shown as a separate contour map and three-dimensional surface on the same $(\bar{q}, \bar{\omega})$ grid.
...
In the contour panels, each channel is rendered on a base-10 logarithmic scale with
$S_{\max}$ set by the grid maximum and
$S_{\min}=\max\!\left(S_{>0},\,10^{-12}\right)$,
where $S_{>0}$ is the smallest positive amplitude on the $(\bar{q},\bar{\omega})$ mesh.
The color field uses $N_{\mathrm{fill}}=200$ uniformly log-spaced levels and the black isolines
$N_{\mathrm{line}}=72$ uniformly log-spaced contours,
$S_k = S_{\min}\left(S_{\max}/S_{\min}\right)^{k/N}$,
so that successive contours are separated by equal increments in $\log_{10} S$
rather than in linear spectral weight.

Three features stand out.
First, $S_{ee}(\bar{q}, \bar{\omega})$ [Figs.~\ref{fig:S_ee} and~\ref{fig:S_ee_3d}] exhibits a sharp plasmon peak in the high-frequency, high-momentum sector.
...
Second, $S_{ii}(\bar{q}, \bar{\omega})$ [Figs.~\ref{fig:S_ii} and~\ref{fig:S_ii_3d}] is confined to the extreme low-frequency band ($\bar{\omega} \rightarrow 0$),
...
and the structure factor weight concentrates in the acoustic window rather than tracking the electron plasmon dispersion.

Third, $S_{ei}(\bar{q}, \bar{\omega})$ [Figs.~\ref{fig:S_ei} and~\ref{fig:S_ei_3d}] provides the decisive smoking gun for intercomponent screening and entanglement.
...
Instead, the engine employs a pure, stateless Brent method on the bracketed interval,
which evaluates $\Re \varepsilon$ only through point values supplied by the Lindhard--RPA pipeline.

The search bracket is physically anchored by the classical Bohm--Gross long-wavelength dispersion relation~\cite{Bohm-Gross1949}. As depicted in Fig.~\ref{fig:plasmon-dispersion}, the deterministically extracted plasmon branch diverges cleanly from this classical limit due to exact quantum screening effects, ultimately terminating with abrupt precision as it merges into the single-particle continuum. Confining the initial frequency scan to this physically informed collective sector prevents the root finder from locking onto unphysical sign changes or spurious resonances that routinely plague Matsubara-contaminated dielectric functions.
This prevents the root finder from locking onto unphysical sign changes
associated with non-plasmon structure or with the spurious resonances
that arise when Matsubara or ensemble-mixed routes contaminate the dielectric function.
Upon convergence, the Landau damping coefficient
$\Im \varepsilon \left( \bar{q}, \bar{\omega}_p \right)$
is evaluated at the same root using the identical complex $\varepsilon$,
...
Legacy implementations that evaluate $\Re \chi^L$ through divergent Matsubara pole summations,
Euler--Maclaurin hybrids, or ensemble-mixed analytic continuations
...
the canonical $T = 0$ Lindhard analytic solution summarized in Appendix~\ref{sec:Lindhard-function},
...
Figures~\ref{fig:t0-re-2d} and \ref{fig:t0-im-2d} visualize the exact analytic topology of this $T=0$ reference over the $(\bar{q}, \bar{\omega})$ plane, where the logarithmic singularity in the real part forms a sharp topological ridge. When the numerical extraction is compared along the $\bar{q}=1$ slice [Fig.~\ref{fig:t0-1d-slice}], the engine output (markers) tracks a well-defined, repeatable trajectory, while the Appendix~\ref{sec:Lindhard-function} reference (solid curves) occupies a distinct manifold. As shown in the error analysis of Fig.~\ref{fig:t0-1d-slice}, the absolute errors in $\Re \chi^L$ remain at the $\mathcal{O}(1)$ level, whereas the imaginary errors are confined to the truncation floor of the sinc evaluation.

This $\mathcal{O}(1)$ offset must not be interpreted as a numerical artifact or a defect of the MosaiQ-Lindhard engine. Rather, it is the inescapable, operational signature of the thermodynamic inconsistency diagnosed in Sec.~\ref{sec:finite-temperature-Lindhard}: legacy $T = 0$ analytical formulas assemble $\Re \chi$ and $\Im \chi$ from thermodynamically distinct prescriptions---canonical contour sums or zero-temperature limits for the real part, and grand-canonical dissipative kernels for the imaginary part---that are not jointly constrained by a single Kramers--Kr\"onig relation.
...
The legacy Appendix~\ref{sec:Lindhard-function} $T = 0$ solution cannot be smoothly connected
to the finite-temperature grand-canonical dissipation through that Hilbert transform;
forcing a numerical code to match the Appendix~\ref{sec:Lindhard-function} real part at $\tau \rightarrow 0$
...
and the observed offset against textbook $T = 0$ formulas is the expected price of refusing
...
Remarkably, our engine captures this profound phase-transition precursor purely through the causality-constrained Kramers--Kr\"onig integration of the fundamental quantum response, entirely independent of ad hoc bridge functions or phenomenological liquid-state integral equations.
...
that have served for decades as consistency standards for dynamic structure factors~\cite{Pines-Nozieres1989}.
...
without the distortions introduced by probabilistic shortcuts, illegal Fubini interchanges,
or ensemble-mixed response functions.
It is the reference against which legacy codes---and the textbooks that justified them---must now be judged.

\section*{Data and Code Availability}
The deterministic C++20 engine (MosaiQ-Lindhard) and the associated Python visualization scripts used to generate the results in this study are open-source and publicly available. To facilitate independent verification of the macroscopic sum rules, the deterministic plasmon dispersion extraction, and the mathematical breakdown of the legacy contour-integration method at the highly degenerate $T \to 0$ limit, readers are strongly encouraged to clone the repository and reproduce the findings. The complete source code, test suites, and build instructions can be accessed at: \url{https://github.com/BonaSapiensInc/MosaiQ-Lindhard}.
...
for performing the Kramers-Kronig relation Eq.~(\ref{Kramers-Kronig-relation}).
...
\section{Reduction to the Zero-Temperature Lindhard Function via Kramers--Kr\"onig Relation}
\label{sec:Lindhard-function}

One of the central claims of this work is that the finite-temperature formulation maintains exact causality. To demonstrate this analytic continuity explicitly, we show that the well-known zero-temperature Lindhard function can be recovered purely from the imaginary part through the Kramers--Kr\"onig relation, without invoking contour integration in the complex plane.

In the limit \(\tau \to 0\), the Fermi--Dirac distribution becomes the Heaviside step function \(n(\kappa) \to \Theta(1 - \kappa)\). Legacy approaches routinely attempt to substitute this step function back into the contour integration. However, \(\Theta(1-\kappa)\) is a distribution that is strictly non-analytic (non-holomorphic) across the complex \(\kappa\)-plane. By definition, it violates the fundamental prerequisite of Cauchy's residue theorem, which requires the integrand to be analytic everywhere within the closed contour except at isolated singularities. Therefore, extending the \(T=0\) step-function Lindhard integral into the complex plane is a mathematically illegal operation from the very beginning.

Instead, evaluating the causality-constrained imaginary part strictly on the real axis yields the standard piecewise form~\cite{Lindhard1954}:
\begin{equation}
\frac{\Im \chi^L(\bar{q}, \bar{\omega}, 0)}{\hbar D(\epsilon_\mathrm{F})} = \frac{\pi}{4 \bar{q}}
\begin{cases}
0 , & \xi_-^2 \ge 1 \\
...
\end{cases}
\end{equation}
Applying the Kramers--Kr\"onig relation directly to this causal dissipative part,
\begin{equation}
\Re \chi^L(\bar{q}, \bar{\omega}, 0) = \frac{1}{\pi} \mathcal{P} \int_{-\infty}^{\infty} 
\frac{\Im \chi^L(\bar{q}, \bar{\omega}', 0)}{\bar{\omega}' - \bar{\omega}} \, d\bar{\omega}'.
\end{equation}
By substituting the piecewise imaginary part into the Kramers--Kr\"onig relation, the principal-value integral naturally splits into strictly finite domains bounded by \(\bar{\omega}'\) where \(\xi_{\pm}^2(\bar{\omega}') = 1\). The integration requires no complex contour closures, but merely the evaluation of elementary rational functions on the real axis:
\begin{equation}
\Re \chi^L(\bar{q}, \bar{\omega}, 0) = \frac{1}{\pi} \left[ \int_{\Delta_1} \frac{\frac{\pi}{4\bar{q}} \bar{\omega}'}{\bar{\omega}' - \bar{\omega}} d\bar{\omega}' + \int_{\Delta_2} \frac{\frac{\pi}{4\bar{q}} (1 - \xi_-^2(\bar{\omega}'))}{\bar{\omega}' - \bar{\omega}} d\bar{\omega}' \right] ,
\end{equation}
where the intervals \(\Delta_1\) and \(\Delta_2\) correspond to the piecewise supports defined in Eq.~(D1). Through straightforward partial fraction decomposition, the singularities at \(\bar{\omega}' = \bar{\omega}\) yield the exact logarithmic terms without any ad-hoc cancellations:
\begin{equation}
\int \frac{d\bar{\omega}'}{\bar{\omega}' - \bar{\omega}} \rightarrow \ln|\bar{\omega}' - \bar{\omega}| ,
\end{equation}
which, upon evaluating the boundaries of \(\Delta_1\) and \(\Delta_2\), algebraically reorganizes into the standard real-part expression:
\begin{equation}
\frac{\Re \chi^L(\bar{q}, \bar{\omega}, 0)}{\hbar D(\epsilon_\mathrm{F})} = \frac{1}{2\bar{q}} \left[ \bar{q} 
+ \frac{1}{2} \left( 1 - \xi_-^2(\bar{\omega}) \right) \ln\left\vert \frac{\xi_-(\bar{\omega}) - 1}{\xi_-(\bar{\omega}) + 1} \right\vert
+ \frac{1}{2} \left( 1 - \xi_+^2(\bar{\omega}) \right) \ln\left\vert \frac{\xi_+(\bar{\omega}) - 1}{\xi_+(\bar{\omega}) + 1} \right\vert
\right] .
\end{equation}

This derivation is performed entirely on the real axis. No analytic continuation into the complex \(\kappa\)- or \(\omega\)-plane is required, thereby avoiding the analytic inconsistency that arises when one attempts to represent the step function \(\Theta(1-\kappa)\) via its Fourier integral.

\textbf{Methodological Remark on Legacy Approaches.} 
While Jordan's lemma is routinely assumed to hold in many contour-based calculations in theoretical physics under the implicit faith that physical quantities naturally converge, the specific analytic structure of the Fermi--Dirac factor (and its zero-temperature step-function limit) violates the necessary decay condition in substantial portions of the complex plane. Our real-axis formulation avoids this subtlety entirely. The fact that the correct \(T=0\) real part can be reconstructed solely from the imaginary part via the Kramers--Kr\"onig transform underscores that our causality-constrained framework is not only consistent with the established zero-temperature result, but also provides the physically rigorous route for finite temperature, free from the systemic ensemble inconsistencies of legacy contour methods.
...
Jordan's Lemma \cite{Jordan1894,BrownChurchill2009} guarantees that the arc integral vanishes
...
\paragraph{Divergence of the Matsubara-series tail and illegal order interchange.}
...
\paragraph{Physical Implications of the Divergent Legacy Method.}
The mathematical divergence derived above is the exact manifestation of the broken causality and statistical breakdown inherent in the contour integration. By evaluating the thermodynamic response at the discrete Matsubara poles, the legacy method replaces the physical continuous density of states with a discrete superposition of states at complex energies, $E_j = \mu \pm i\pi(2j+1)k_B T$. This mathematical maneuver forces the system into an ensemble fragmentation---an unphysical mixture of infinitely many non-Hermitian thermal configurations. The resulting divergent tails are not merely mathematical artifacts; they are the severe thermodynamic penalty for violating the singular integrity of the grand canonical ensemble.
...
\begin{enumerate}
    \item \textbf{The Illusion of Meromorphic Continuation (Sinc-Quadrature):} While successfully utilized for the Kramers--Kr\"onig transform in Sec.~\ref{sec:finite-temperature-Lindhard}, applying Sinc-quadrature to the $\bar{\omega}$ integral forces the evaluation across the massive branch cuts generated by the $\coth$ and logarithmic terms. Ignoring these cuts and summing only the residues results in catastrophic topological failure, causing $S(\bar{q})$ to artificially diverge.
    \item \textbf{The Blind Spot of Trigonometric Mapping:} Mapping the infinite domain $(-\infty, \infty)$ into a finite interval $(-\pi/2, \pi/2)$ via $x = \tan(\theta)$ eliminates the truncation error mathematically. However, physically, the plasmon peak and single-particle continuum shift to $\bar{\omega} \sim \bar{q}^2 + 2\bar{q}$ at large $\bar{q}$. This mapping crushes these massive high-frequency spectral mountains into a microscopically thin sliver just below $\pi/2$. Adaptive integrators interpret this crushed region as a flat tail, causing \emph{false convergence} and missing the spectral weight entirely.
    \item \textbf{The Broken Smoothness (Tanh-Sinh and Gauss-Hermite):} Methods relying on exponential decay or absolute analyticity degrade severely in the highly degenerate limit ($T \to 0$), where $\Im \chi^L$ forms a near-discontinuous kink at the single-particle boundaries.
    \item \textbf{Grid Resonance and the 0/0 Singularity Trap:} When the kinematic variable $\xi_-$ perfectly aligns with a quadrature node (e.g., $s=0$), the Cauchy Principal Value kernel $[1 - \cos(\pi(s - \xi_-)/h)]/(s - \xi_-)$ encounters a $0/0$ singularity. Without rigorous handling via L'H\^opital's rule (which yields exactly $0$), the floating-point environment produces a \texttt{NaN}, instantly evaporating the dominant spectral peak and artificially crashing $S_{ee}(\bar{q} \to \infty) \to 0$. Our engine applies exact analytic limits at these resonance nodes at the bare-metal level, completely eliminating the need for ad-hoc grid offsets or smearing.
\end{enumerate}
...
see also the discussion of Jordan's Lemma in Ref.~\cite{BrownChurchill2009}.

\bibitem{BrownChurchill2009} J. W. Brown and R. V. Churchill,
\textit{Complex Variables and Applications}, 8th ed. (McGraw-Hill, New York, 2009) Sec.~5.
...
\bibitem{Arfken-Weber1995} G. B. Arfken and H. J. Weber, \textit{Mathematical Methods for Physicists Fourth Edition}
(Academic Press, San Diego, 1995) Chap. 10.
...
\begin{tikzpicture}
 ...
 % simple poles from the generic integrals
  ...
  % simple poles from the Fermi-Dirac distribution
  \node at (1.2, 1.0) {$\bullet$};
  \node at (1.7, 1.5) {$\bullet$};
  \node at (2.3, 2.0) {$\bullet$};
  \node at (2.8, 2.5) {$\bullet$};
  \node at (3.3, 3.0) {$\bullet$};
  \node at (2.2, 2.6) {${\kappa_j^+}$};
  ...
  \draw[thick, decoration = { markings,
    ...
    -- (1.8,0) arc(180:360:0.2) -- (2.8,0) arc(180:360:0.2) -- (5,0) arc(0:180:5);
\end{tikzpicture}
\end{center}
\caption{The schematic representations of 
...
% simple poles
  ...
  \node at (1.5, -0.25) {$\bullet$};
  \node at (1.5, 0.50) {$\bullet$};
  \node at (1.5, -0.50) {$\bullet$};
  \node at (1.5, 0.75) {$\bullet$};
  \node at (1.5, -0.75) {$\bullet$};
  \node at (1.5, 1.00) {$\bullet$};
  \node at (1.5, -1.00) {$\bullet$};
  \node at (1.5, 1.25) {$\bullet$};
  \node at (1.5, -1.25) {$\bullet$};
  \node at (1.5, 1.50) {$\bullet$};
  \node at (1.5, -1.50) {$\bullet$};
  \node at (1.5, 1.75) {$\bullet$};
  \node at (1.5, -1.75) {$\bullet$};
  \node at (1.5, 2.00) {$\bullet$};
  \node at (1.5, -2.00) {$\bullet$};
  \node at (1.5, 2.25) {$\bullet$};
  \node at (1.5, -2.25) {$\bullet$};
  \node at (1.5, 2.50) {$\bullet$};
  \node at (1.5, -2.50) {$\bullet$};
  \node at (1.5, 2.75) {$\bullet$};
  \node at (1.5, -2.75) {$\bullet$};
...
\end{tikzpicture}
\end{center}
\caption{Schematic positions of the infinite series of simple poles 
...
\label{Fermi-Dirac-contour-integral}
\end{figure}
...
\begin{subfigure}[t]{0.33\textwidth}
  \centering
  \includegraphics[width=\linewidth]{../output/S_ee_contour.pdf}
  \caption{$S_{ee}$ contour map.}
  \label{fig:S_ee}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.33\textwidth}
  \centering
  \includegraphics[width=\linewidth]{../output/S_ee_3d.pdf}
  \caption{$S_{ee}$ surface.}
  \label{fig:S_ee_3d}
\end{subfigure}
...
\begin{subfigure}[t]{0.33\textwidth}
  \centering
  \includegraphics[width=\linewidth]{../output/S_ii_contour.pdf}
  \caption{$S_{ii}$ contour map.}
  \label{fig:S_ii}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.33\textwidth}
  \centering
  \includegraphics[width=\linewidth]{../output/S_ii_3d.pdf}
  \caption{$S_{ii}$ surface.}
  \label{fig:S_ii_3d}
\end{subfigure}
...
\begin{subfigure}[t]{0.33\textwidth}
  \centering
  \includegraphics[width=\linewidth]{../output/S_ei_contour.pdf}
  \caption{$S_{ei}$ contour map.}
  \label{fig:S_ei}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.33\textwidth}
  \centering
  \includegraphics[width=\linewidth]{../output/S_ei_3d.pdf}
  \caption{$S_{ei}$ surface.}
  \label{fig:S_ei_3d}
\end{subfigure}

\caption{Dynamic structure factors from the MosaiQ-Lindhard deterministic sandbox at $r_s = 1.0$ and $T = 10{,}000\,\mathrm{K}$.
Panels~\textbf{(a)}--\textbf{(f)} display the electron--electron, ion--ion, and electron--ion channels, with each row pairing a contour map (left) and the matching three-dimensional surface (right) on the same $(\bar{q}, \bar{\omega})$ grid.
Each contour map maps $|S_{st}|$ on a base-10 logarithmic scale from $S_{\min}=\max(S_{>0},10^{-12})$ to $S_{\max}$ (the channel maximum on the grid).
The filled color field uses $N_{\mathrm{fill}}=200$ log-uniform levels and the black isolines $N_{\mathrm{line}}=72$ log-uniform contours, $S_k=S_{\min}(S_{\max}/S_{\min})^{k/N}$, so that successive isolines are equally spaced in $\log_{10} S$; the companion surface panels share the same limits and colormap.}
\label{fig:structure-factors}
\end{figure}

\clearpage
\begin{figure}[htbp]
\centering
\includegraphics[width=0.72\textwidth]{../output/plasmon_dispersion.pdf}
\caption{Extracted electron plasmon dispersion trajectory (top) and corresponding Landau damping (bottom)
for a partially degenerate plasma at $r_s = 2.0$ and $T = 10{,}000\,\mathrm{K}$.
The dispersion (solid line) diverges from the classical Bohm--Gross limit~\cite{Bohm-Gross1949} (dashed line) due to quantum screening,
and terminates abruptly as it merges with the single-particle continuum
where Landau damping $\Im[\varepsilon]$ grows exponentially.
Roots were deterministically extracted via Brent's method on the causality-constrained $\Re[\varepsilon]$ manifold.}
\label{fig:plasmon-dispersion}
\end{figure}
...
\begin{subfigure}[b]{0.40\textwidth}
  \centering
  \includegraphics[width=\linewidth]{../output/t0_analytic_re_contour.pdf}
  \caption{Analytic $\Re \chi^L_{T=0}$ (Eq.~D5).}
  \label{fig:t0-re-2d}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.40\textwidth}
  \centering
  \includegraphics[width=\linewidth]{../output/t0_analytic_im_contour.pdf}
  \caption{Analytic $|\Im \chi^L_{T=0}|$ (Eq.~D1).}
  \label{fig:t0-im-2d}
\end{subfigure}
...
\begin{subfigure}[b]{0.62\textwidth}
  \centering
  \includegraphics[width=\linewidth]{../output/t0_limit_validation.pdf}
  \caption{1D slice validation at $\bar{q} = 1.0$ and error analysis.}
  \label{fig:t0-1d-slice}
\end{subfigure}

\caption{Highly degenerate limit validation at extreme degeneracy ($\tau = 10^{-4}$, $T \approx 11\,\mathrm{K}$). \textbf{(a)--(b)} The exact $T = 0$ Lindhard susceptibility topology over the $(\bar{q}, \bar{\omega})$ plane, derived via Kramers--Kr\"onig relation in Appendix~\ref{sec:Lindhard-function}. The white dashed line marks the $\bar{q} = 1.0$ trajectory. \textbf{(c)} Top panel: 1D extraction along $\bar{q} = 1.0$ comparing the exact analytic reference (solid curves) against the MosaiQ-Lindhard engine (markers). Bottom panel: absolute errors $|\Delta \Re|$ and $|\Delta \Im|$ on a logarithmic scale. While $\Im \chi^L$ flawlessly converges to the $\tau \rightarrow 0$ limit, $\Re \chi^L$ retains an $\mathcal{O}(1)$ offset, visually capturing the ensemble inconsistency of legacy contour methods discussed in Sec.~\ref{sec:finite-temperature-Lindhard}.}
\label{fig:t0-limit-validation}
\end{figure}
...
\begin{subfigure}[b]{0.34\textwidth}
  \centering
  \includegraphics[width=\linewidth]{../output/Sq_gamma_ee.pdf}
  \caption{$S_{ee}(\bar{q})$}
  \label{fig:Sq-gamma-ee}
\end{subfigure}
...
\begin{subfigure}[b]{0.34\textwidth}
  \centering
  \includegraphics[width=\linewidth]{../output/Sq_gamma_ii.pdf}
  \caption{$S_{ii}(\bar{q})$}
  \label{fig:Sq-gamma-ii}
\end{subfigure}
...
\begin{subfigure}[b]{0.34\textwidth}
  \centering
  \includegraphics[width=\linewidth]{../output/Sq_gamma_ei.pdf}
  \caption{$S_{ei}(\bar{q})$}
  \label{fig:Sq-gamma-ei}
\end{subfigure}
\caption{Static structure factors (a) $S_{ee}(\bar{q})$, (b) $S_{ii}(\bar{q})$, and (c) $S_{ei}(\bar{q})$ obtained via direct frequency integration of the dynamic MosaiQ-Lindhard response for a dense plasma at $r_s = 1.0$. In each panel, the temperature is deterministically mapped to sweep the plasma coupling parameter $\Gamma \in \{10, 50, 100, 150\}$. The smooth emergence of spatial correlation peaks at higher $\Gamma$ and the distinct cross-correlation $S_{ei}$ behavior demonstrate that the causality-constrained pipeline unconditionally preserves the macroscopic fluctuation--dissipation constraints across both weakly and strongly coupled regimes, without relying on classical integral-equation approximations.}
\label{fig:Sq-gamma}
\end{figure}
